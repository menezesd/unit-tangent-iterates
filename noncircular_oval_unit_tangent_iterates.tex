...
iterate \(\Tmap^n\Gamma_0\) is an oval.  This answers a question of
Tabachnikov \cite[Conjecture~4]{Tabachnikov2023} in the negative.  The
...
\emph{steering angle}.  This is the classical bicycle correspondence; see
\cite{BorLeviPerlineTabachnikov2020,Foote1998,FooteLeviTabachnikov2013,Tabachnikov2006}
for its geometry and history.  A prescribed front does not determine a rear until the
...
\emph{Formalization.}
The complete proof of Theorem~\ref{thm:main} has been formalized and
machine-checked in the Lean~4 proof assistant on top of the Mathlib
library.  The formal development constructs every analytic input---the
translating hairpin, the exact two-cap pairs, the curvature-measure
matching, and the backward-shadowing limit---and derives the theorem
unconditionally, with no unproved assumptions and no axioms beyond Lean's
...
\((0,\pi)\) and the uniform barrier bounds on that open interval
(\(f_\eps^-\le f\le f_\eps^+\) on \((0,\pi)\) only; the smooth extension
off a collar, identically \(1\), is not required to satisfy the lower
barrier).  The constants \(C_j,D_j\) below depend on the fixed
\(\eps\) (quantifier order: first choose \(\eps\le1/40\), so that the
curvature \(\sin\theta/f_\eps\le1/(\eps^{-1}-\eps)\le1/20\); then obtain
\(C_j(\eps),D_j(\eps)\)); no uniform control of \(\eps D_j(\eps)\) over
\(\eps\) is asserted.
...
For the \emph{fixed} wide hairpin above, the function \(y\) and all its
derivatives decay exponentially at both ends.  More precisely, there are
\(a_-,a_+>0\) such that, for every \(j\geq0\) (with \(C_j=C_j(\eps)\),
\(D_j=D_j(\eps)\) depending on the fixed \(\eps\)),
\[
...
\[
   \|\Theta_H-\Theta_*\|_{L^\infty(I_H)}
      \leq CH e^{-\beta H}.
\]
Consequently, for any \(0<\beta'<\beta\) and all large \(H\),
\(H\|\Theta_H-\Theta_*\|_\infty\le CH^2e^{-\beta H}
\le C'e^{-\beta' H}\le1\), so
\[
\begin{aligned}
   W_H
...
\(0\le\kappa^{(i)}\le\kappa_*<1\) for \(i=0,1\) (nonnegativity suffices;
strict positivity is not required).  There is a constant-speed
(\(g\equiv1\)) path \(\Gamma\) from \(G_0\) to \(G_1\) whose curvature is at most
...
\end{equation}
The constant \(C\) depends only on \(\kappa_*\) and not on \(L\).
\end{lemma}
...
In the shadowing scheme of Theorem~\ref{thm:shadow}, the factor
\((1+L)^2\) in \eqref{eq:interp-estimate} is carried by the defect
quantities \(e_n\); the tube constants \(M_0\), \(M_1\),
\(C_{\mathrm{tube}}\) introduced there depend only on the curvature
ceiling.
...
\(C^r\), \(r\geq2\), the rear is \(C^{r+1}\).  Moreover the steering angle
is \(2\)-Lipschitz in front arclength (\(|\delta'|=|K-\sin\delta|\le2\)),
and the inverse of the rear-arclength correspondence \(x=x(s)\), the front
arclength \(\sigma=x^{-1}\) as a function of rear arclength, satisfies
\[
   |\sigma(u)-\sigma(v)|\le\frac{|u-v|}{\sqrt{1-\widehat\kappa^2}}.
\]
\end{lemma}
...
regular and has nonnegative curvature \(\tan\delta\); inverting this
bound gives the Lipschitz estimate for \(\sigma=x^{-1}\), and the
\(2\)-Lipschitz bound for \(\delta\) is immediate from the steering
equation.  When the perimeter
...
For each \(n\), let \(\delta_{n,N}\) and \(\sigma_{n,N}\) denote the
steering angle and the inverse rear-arclength correspondence of the
terminal pullback \(Z_n^{(N)}\).  By the Lipschitz bounds of
Lemma~\ref{lem:closed-strip-inverse}, the families \(\{\delta_{n,N}\}_N\)
and \(\{\sigma_{n,N}\}_N\) are uniformly bounded and equicontinuous, so a
diagonal Arzelà--Ascoli extraction gives one subsequence of terminal
indices along which both converge uniformly for every \(n\); the limits
satisfy the same steering ODE and right-inverse identity.  Marked
geometric \(C^2\) convergence, strengthened in this way to \(C^0\)
convergence of the steering data, therefore implies
\[
   \kappa_{X_n}\geq0.
\]
...
\newblock \href{https://doi.org/10.1093/imrn/rny087}
{doi:10.1093/imrn/rny087}.

\bibitem{Foote1998}
R.~L.~Foote,
\newblock Geometry of the Prytz planimeter,
\newblock \emph{Reports on Mathematical Physics} \textbf{42} (1998),
no.~1--2, 249--271,
\newblock \href{https://doi.org/10.1016/S0034-4877(98)80013-X}
{doi:10.1016/S0034-4877(98)80013-X}.

\bibitem{FooteLeviTabachnikov2013}
R.~L.~Foote, M.~Levi, and S.~Tabachnikov,
\newblock Tractrices, bicycle tire tracks, hatchet planimeters, and a
100-year-old conjecture,
\newblock \emph{American Mathematical Monthly} \textbf{120} (2013),
no.~3, 199--216,
\newblock \href{https://doi.org/10.4169/amer.math.monthly.120.03.199}
...
\bibitem{Tabachnikov2006}
S.~Tabachnikov,
\newblock Tire track geometry: variations on a theme,
\newblock \emph{Israel Journal of Mathematics} \textbf{151} (2006),
1--28,
\newblock \href{https://doi.org/10.1007/BF02777353}
