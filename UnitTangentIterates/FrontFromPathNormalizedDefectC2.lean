...
/-!
# The path pseudodistance for the path itself, together with the C² defect of its
gauge marking
...
read — `Φ` fixes the base point, reads exactly one rear period, deviates from the
affine marking of the terminal period by at most `2 P₁ κ̂/(1 − κ̂²) · cost Γ`, and
leaves the terminal marked curve within `markingC2Bound` of the marked reference
curve `b` in the `C²` metric — under the geometric hypothesis, already used to
fix the base point in the original statement, that the path does not move at its
marked point.

Main result: `pathDist_and_distC2_le_of_path_normalized`.
-/
...
namespace FrontFromPathNormalizedDefectC2
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  FrontFromPath MarkingDeviationC2 MarkingFlowDefectC2 RearOwnTangentialCostC2
...
front data on the normalized circle, together with the C² defect of its gauge
marking.**  `FrontFromPathNormalized.pathDist_le_of_path_normalized` with the
extra conclusions about the marking. -/
theorem pathDist_and_distC2_le_of_path_normalized {p q : Data} (Γ : NormalPath p q) (p' b : Data)
  ...
  refine RearOwnPathDistNormalizedDefectC2.pathDist_and_distC2_le_of_front_normalized
    Γ p' b hP0 hkh0 hkh1 hPl hPu
...
end FrontFromPathNormalizedDefectC2
