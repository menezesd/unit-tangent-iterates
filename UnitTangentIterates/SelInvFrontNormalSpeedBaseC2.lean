...
`SelInvFrontClosingBaseC2.dist_selInv_le_of_front_closing_base_C2` still carries the
...
`dist_selInv_le_of_front_normalSpeed_base_C2`.
-/
...
namespace SelInvFrontNormalSpeedBaseC2

open SelInvFrontNormalSpeedFundamentalDriftC2
...
`SelInvFrontClosingBaseC2.dist_selInv_le_of_front_closing_base_C2` with that derivative
and its bound replaced by the joint requirement that the normal speed be `C¹`
in the parameter. -/
theorem dist_selInv_le_of_front_normalSpeed_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact SelInvFrontClosingBaseC2.dist_selInv_le_of_front_closing_base_C2
...
end SelInvFrontNormalSpeedBaseC2
