...
`SelInvFrontJacobiBaseC2.dist_selInv_le_of_front_jacobi_base_C2` still carries the
...
`rearOwnVelocity` names it, and `dist_selInv_le_of_front_velocity_base_C2` is the
...
`dist_selInv_le_of_front_velocity_base_C2`.
-/
...
namespace SelInvFrontVelocityBaseC2

open SelInvFrontVelocityFundamentalDriftC2
...
are those of `SelInvFrontJacobiBaseC2.dist_selInv_le_of_front_jacobi_base_C2` with the
velocity replaced everywhere by the canonical `rearOwnVelocity` and the
hypothesis identifying it removed. -/
theorem dist_selInv_le_of_front_velocity_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact SelInvFrontJacobiBaseC2.dist_selInv_le_of_front_jacobi_base_C2 (Ydot := rearOwnVelocity Γ.X V A P δ sf)
