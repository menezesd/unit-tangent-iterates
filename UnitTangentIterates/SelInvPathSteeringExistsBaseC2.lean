...
`SelInvPathTaylorBaseC2.dist_selInv_le_of_path_taylor_base_C2` still takes the selected
...
Main result: `exists_steering_dist_selInv_le_of_path_base_C2`.
-/
...
namespace SelInvPathSteeringExistsBaseC2

open SelInvPathSteeringExistsFundamentalDriftC2
...
those of `SelInvPathTaylorBaseC2.dist_selInv_le_of_path_taylor_base_C2` with the
...
theorem exists_steering_dist_selInv_le_of_path_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  refine ⟨dn, fun t s => dn t (s / P t), hdnper, hstrip, hsol, fun _ _ => rfl,
    fun Pv0 khat hkappa1 hnumA hnumK hsmall => ?_⟩
  have hKnbd : ∀ t σ, |Kn t σ| ≤ kh := fun t σ =>
    abs_le.mpr ⟨by linarith [hKn0 t σ, hkh0], hKnk t σ⟩
  exact SelInvPathTaylorBaseC2.dist_selInv_le_of_path_taylor_base_C2 (dn := dn) (δ := fun t s => dn t (s / P t))
...
end SelInvPathSteeringExistsBaseC2
