...
/-!
# The last link: the comparison path of the `C²` estimate is the family of
selected rears itself
...
tangential component of that motion.
...
Main result: `dist_selInv_le_of_rear_family_C2`.
-/
...
namespace SelInvMarkingDefectRearFamilyC2
...
theorem dist_selInv_le_of_rear_family_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  ------------------------------------------------------------------
  obtain ⟨Psi, hPsi0, hPsiflow, hrmain⟩ :=
    exists_variableSpeed_normalPath_of_rearFamily (F := frontOfPath Γ.X P)
      (Θ := angleOfPath V A P) (δ := δ) (sf := sf) (K := curvOfPath V A P) (Ydot := Ydot)
      (etaF := etaF) (P := P) (Q := fun t => rearArclength (δ t) (P t))
      (ell := rearArclength (δ 0) (P 0)) (khat := khat) (alphaT := alphaT) (kT := kT)
      (Kx := Kx) (kx := kx) (Rb := Rb) (r := rr) (gS := gS) (Dd := Dd) (d := dd)
      (m := m) (P0 := Pv0)
      Γ hkh0 hkh1 (hQpos 0) hstrip0 hstrip1 hK hF hΘ hsteer hsfspace hcos hYt hFC hΘC
      hδC hsfC hYdotC hangC hkC1 hQpos hper hjac hPpos hlink hkappa1 halphaT hkT halphaTc
      hkTc halphaTS hmixed hKxbd hKxnn hKxm hkXc hRbd hRbm hr hgSd hgSbd hDm hmc hm0
      hmstop hmge hnumA hnumK
  ------------------------------------------------------------------
...
end SelInvMarkingDefectRearFamilyC2
