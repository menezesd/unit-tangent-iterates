...
/-!
# The `C²` estimate with the tangential drift discharged, with no rigidity

`SelInvFrontSourceFundamentalC2.dist_selInv_le_of_front_source_fundamental_C2`
still asks the caller for a bound `Rb` on the tangential drift

`ξ(t,x) = ⟨∂_t Y(t,x), e^{iΨ(t,x)}⟩`

of the family of selected rear tracks written in its own arclength — but only
for `x` in one rear period `[0, Q t]`.
`RearOwnDriftFundamental.abs_frameTangential_le_cost_on_period` produces exactly
such a bound from the front data: since the rear normal velocity is controlled
by the front's through the maximum principle for the inverse Jacobi ODE, and
`∂_xξ = η · tan δ`, a drift vanishing at the marked point obeys

`|ξ(t,x)| ≤ (Q(t) · κ̂/(1−κ̂²)) · m t`   for `x ∈ [0, Q t]` ,
...
explicit `P₁ · κ̂/(1−κ̂²)`, and all that is asked of the family of rears in its
place is that its tangential drift vanish at the marked point.  Unlike
`SelInvFrontDriftC2.dist_selInv_le_of_front_drift_C2`, **no periodicity of the
drift — equivalently, no rigidity of the rear arclength period — is assumed.**

Main result: `dist_selInv_le_of_front_drift_fundamental_C2`.
-/
...
namespace SelInvFrontDriftFundamentalC2
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  SelInvFrontSourceFundamentalC2 RearOwnTangentialSup
...
`SelInvFrontSourceFundamentalC2.dist_selInv_le_of_front_source_fundamental_C2`,
with the bound `Rb` on the tangential drift and its constant `rr` replaced by
the vanishing of the drift at the marked point alone; the constant of the
estimate is then the explicit `P₁ · κ̂/(1−κ̂²)`.  Nothing is assumed about the
motion of the rear arclength period. -/
theorem dist_selInv_le_of_front_drift_fundamental_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    -- the tangential drift of the selected rears rests at the mark
    (hxizero : ∀ t,
  ...
  have hPC1 : ContDiff ℝ (1 : ℕ) P := by simpa using hPC4.of_le hle14
  have hRbd : ∀ t, ∀ x ∈ Icc (0 : ℝ) (rearArclength (δ t) (P t)),
      |frameTangential Ydot (rearOwnAngle (angleOfPath V A P) δ sf) t x|
        ≤ (P1 * (kh / (1 - kh ^ 2))) * Γ.m t := fun t x hx =>
    RearOwnDriftFundamental.abs_frameTangential_le_cost_on_period (etaF := etaF)
      (m := Γ.m) (K := curvOfPath V A P) (Qmax := P1) hkh0 hkh1 hQpos hQmax hstrip0
      hstrip1 hcosne hFtangent hΘd hsteer hsfspace hsfinv hδper hFper hΘper hFC1 hΘC1
      hδC hsfC hPC1 hYt (hYdotC.of_le (by norm_num)) (hangC.of_le (by norm_num)) hjac
      hetaFbd Γ.m_nonneg hxizero t hx
  exact SelInvFrontSourceFundamentalC2.dist_selInv_le_of_front_source_fundamental_C2
    ...
    hRbd (fun _ => le_rfl) hrr hxizero hgSd hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontDriftFundamentalC2
