...
`SelInvPathTaylorC2.dist_selInv_le_of_path_taylor_C2` still takes the selected
...
Main result: `exists_steering_dist_selInv_le_of_path_C2`.
-/
...
namespace SelInvPathSteeringExistsC2
...
those of `SelInvPathTaylorC2.dist_selInv_le_of_path_taylor_C2` with the
...
theorem exists_steering_dist_selInv_le_of_path_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_path_taylor_C2 (dn := dn) (δ := fun t s => dn t (s / P t))
...
end SelInvPathSteeringExistsC2
