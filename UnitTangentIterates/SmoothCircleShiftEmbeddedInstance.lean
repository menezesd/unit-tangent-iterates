...
`SelectedInverseShiftEmbedded.exists_steering_pathDistShift_selInv_le_embedded`
...
Main result: `circlePath_shift_instance_embedded`.
-/
...
namespace SmoothCircleShiftEmbeddedInstance
...
/-- **The tangent angle of a marked circle turns by `2π` over one period.** -/
theorem circleData_turning (t : ℝ) :
    ∃ Θ' K' : ℝ → ℝ,
      (∀ s, HasDerivAt (ev (circleData (rad t)))
        (Complex.exp (Complex.I * (Θ' s : ℂ))) s) ∧
      (∀ s, HasDerivAt Θ' (K' s) s) ∧
      (∀ s, Θ' (s + perim (circleData (rad t))) = Θ' s + 2 * Real.pi) := by
  have hr : 0 < rad t := rad_pos t
  refine ⟨SelectedInverseCircle.circleAngle (rad t), fun _ => 1 / rad t, ?_, ?_, ?_⟩
  · rw [SelectedInverseTubeCircle.ev_circleData hr]
    exact SelectedInverseCircle.hasDerivAt_circleFront hr
  · exact fun s => SelectedInverseCircle.hasDerivAt_circleAngle s
  · intro s
    rw [perim_circleData hr, SelectedInverseCircle.circleAngle,
      SelectedInverseCircle.circleAngle]
    field_simp
    ring
...
theorem circlePath_shift_instance_embedded :
...
end SmoothCircleShiftEmbeddedInstance
