...
for three pieces of data attached to the family of selected rears which are in
fact determined by the front:
...
Main result: `dist_selInv_le_of_front_strip_C2`.
-/
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  SelInvFrontSourceC2 RearOwnTangential
...
`2κ̂/(1−κ̂²)^{3/2}`: on the selected strip `|K| ≤ κ̂`, `0 ≤ sin δ ≤ κ̂` and
`cos δ ≥ √(1−κ̂²)`, so `|(K − sin δ)/cos³δ| ≤ 2κ̂/(1−κ̂²)^{3/2}`. -/
def stripCurvConst (kh : ℝ) : ℝ := 2 * kh / Real.sqrt (1 - kh ^ 2) ^ 3

theorem stripCurvConst_nonneg {kh : ℝ} (hkh0 : 0 ≤ kh) : 0 ≤ stripCurvConst kh := by
  unfold stripCurvConst
  positivity
...
`SelInvFrontSourceC2.dist_selInv_le_of_front_source_C2`, with the derivative of
...
theorem dist_selInv_le_of_front_strip_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    {Ydot : ℝ → ℝ → ℂ} {etaF alphaT kT : ℝ → ℝ → ℝ} {Rb : ℝ → ℝ}
    {Pv0 khat rr : ℝ}
    (hYt : ∀ t x, HasDerivAt
  ...
  exact dist_selInv_le_of_front_source_C2
    ...
    hRbd hRbm hr hgSex hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontStripC2
