...
namespace ConfiguredBaseProfiledEdgeRecursiveAnalyticSuccessorFamily
...
def rawSource (O : ConfiguredRecursiveEdgeSourceP0ScalarStart.Output MA NA)
    (n : ℕ) :=
...
def rawSpatialFrames
  ...
  exact
    { tangential := R.1
      normal := R.2
      tangential1_bound := (edgeScaledBounds O n).tangential1_bound
      tangential2_bound := (edgeScaledBounds O n).tangential2_bound
      tangential_period_bound := (edgeScaledBounds O n).tangential_period_bound }
...
end ConfiguredBaseProfiledEdgeRecursiveAnalyticSuccessorFamily
