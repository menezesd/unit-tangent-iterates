...
/-!
# The path pseudodistance for the path of fronts itself, together with the C²
...
rear period, deviates from the affine marking of the terminal period by at most
...
Main result: `pathDist_and_distC2_le_of_front_slices`.
-/
...
namespace RearOwnPathDistSlicesDefectC2
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  RearOwnPathDistSlices MarkingDeviationC2 MarkingFlowDefectC2 RearOwnTangentialCostC2

variable {F : ℝ → ℝ → ℂ} {Θ δ : ℝ → ℝ → ℝ} {P : ℝ → ℝ}

/-- **The path pseudodistance of the selected rears for the path of fronts
itself, together with the C² defect of its gauge marking.**
`RearOwnPathDistSlices.pathDist_le_of_front_slices` with the extra
conclusions about the marking. -/
theorem pathDist_and_distC2_le_of_front_slices {p q : Data} (Γ : NormalPath p q) (p' b : Data)
  ...
  exact RearOwnPathDistIntrinsicDefectC2.pathDist_and_distC2_le_of_front_intrinsic Γ p' b
...
end RearOwnPathDistSlicesDefectC2
