...
`SelInvFrontJacobiC2.dist_selInv_le_of_front_jacobi_C2` still carries the
...
`rearOwnVelocity` names it, and `dist_selInv_le_of_front_velocity_C2` is the
...
`dist_selInv_le_of_front_velocity_C2`.
-/
...
namespace SelInvFrontVelocityC2
...
/-- **The velocity of the family of selected rears**, carried in its own
arclength: the parameter derivative of the family of rear tracks of the path of
fronts. -/
def rearOwnVelocity (X V A : ℝ → ℝ → ℂ) (P : ℝ → ℝ) (δ sf : ℝ → ℝ → ℝ) : ℝ → ℝ → ℂ :=
  partialTime (rearOwn (frontOfPath X P) (angleOfPath V A P) δ sf)

/-- **The family of rear tracks is jointly `C¹`**, hence differentiable in the
path parameter, so `rearOwnVelocity` really is its velocity. -/
theorem contDiff_rearOwn_one {X : ℝ → ℝ → ℂ}
    (hFc1 : ContDiff ℝ (1 : ℕ) (uncurry (frontOfPath X P)))
    (hΘc1 : ContDiff ℝ (1 : ℕ) (uncurry (angleOfPath V A P)))
    (hδC : ContDiff ℝ (1 : ℕ) (uncurry δ)) (hsfC : ContDiff ℝ (1 : ℕ) (uncurry sf)) :
    ContDiff ℝ (1 : ℕ) (uncurry (rearOwn (frontOfPath X P) (angleOfPath V A P) δ sf)) := by
  have hsub : ContDiff ℝ (1 : ℕ)
      (uncurry fun t s => angleOfPath V A P t s - δ t s) := by
    simpa [Function.uncurry] using hΘc1.sub hδC
  have hG : ContDiff ℝ (1 : ℕ)
      (uncurry (frontParamTrack (frontOfPath X P) (angleOfPath V A P) δ)) := by
    have heq : uncurry (frontParamTrack (frontOfPath X P) (angleOfPath V A P) δ)
        = fun p : ℝ × ℝ => uncurry (frontOfPath X P) p
          - Complex.exp (Complex.I *
            ((uncurry (fun t s => angleOfPath V A P t s - δ t s) p : ℝ) : ℂ)) := rfl
    rw [heq]
    exact hFc1.sub (contDiff_expI (n := 1) hsub)
  have heq : uncurry (rearOwn (frontOfPath X P) (angleOfPath V A P) δ sf)
      = (uncurry (frontParamTrack (frontOfPath X P) (angleOfPath V A P) δ))
        ∘ fun p : ℝ × ℝ => (p.1, uncurry sf p) := rfl
  rw [heq]
  exact hG.comp (contDiff_fst.prodMk hsfC)

/-- The velocity of the family of selected rears is the parameter derivative of
the family of rear tracks. -/
theorem hasDerivAt_rearOwnVelocity {X : ℝ → ℝ → ℂ}
    (hFc1 : ContDiff ℝ (1 : ℕ) (uncurry (frontOfPath X P)))
    (hΘc1 : ContDiff ℝ (1 : ℕ) (uncurry (angleOfPath V A P)))
    (hδC : ContDiff ℝ (1 : ℕ) (uncurry δ)) (hsfC : ContDiff ℝ (1 : ℕ) (uncurry sf))
    (t x : ℝ) :
    HasDerivAt (fun r => rearOwn (frontOfPath X P) (angleOfPath V A P) δ sf r x)
      (rearOwnVelocity X V A P δ sf t x) t :=
  hasDerivAt_partialTime
    ((contDiff_rearOwn_one hFc1 hΘc1 hδC hsfC).differentiable (by norm_num)) t x
...
are those of `SelInvFrontJacobiC2.dist_selInv_le_of_front_jacobi_C2` with the
velocity replaced everywhere by the canonical `rearOwnVelocity` and the
hypothesis identifying it removed. -/
theorem dist_selInv_le_of_front_velocity_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_front_jacobi_C2 (Ydot := rearOwnVelocity Γ.X V A P δ sf)
