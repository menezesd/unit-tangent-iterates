...
`SelInvPathPerimBaseC2.dist_selInv_le_modulus_of_path_perim_base_C2` asks the caller for
...
Main result: `dist_selInv_le_modulus_of_path_gauge_base_C2`.
-/
...
namespace SelInvPathGaugeBaseC2

open SelInvPathGaugeFundamentalDriftC2
...
`SelInvPathPerimBaseC2.dist_selInv_le_modulus_of_path_perim_base_C2` with the constant
`Pv0` and the two numerical conditions it had to satisfy replaced by the
explicit `pathPv0`. -/
theorem dist_selInv_le_modulus_of_path_gauge_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  obtain ⟨dn, δ, hdnper, hstrip, hsol, hdel, hmain⟩ :=
    SelInvPathPerimBaseC2.dist_selInv_le_modulus_of_path_perim_base_C2 Γ hc hkmin hp hturnp hcq hkminq hq hturnq
      hP0 hkh1 hXC6 hPl hPu hconst hXper hturn hnu hKn0 hKnk hslit hmark
  refine ⟨dn, δ, hdnper, hstrip, hsol, hdel,
    fun khat hkappa1 hsmall => ?_⟩
  ...
  exact hmain (pathPv0 kh P0 khat (P1 * (kh / (1 - kh ^ 2)))) khat hkappa1
    (pathPv0_numA hP0 hkh0 hkhat hr) (pathPv0_numK hP0 hkh0 hkhat hr) hsmall

end SelInvPathGaugeBaseC2
