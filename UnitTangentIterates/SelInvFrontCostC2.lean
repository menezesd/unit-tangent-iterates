...
Main results: `dist_selInv_le_of_front_cost_C2`, `bound_eq_front_modulus`,
`tendsto_selInvFrontModulus_zero`.
-/
...
theorem dist_selInv_le_of_front_cost_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  ------------------------------------------------------------------
  have key := SelInvRearFamilySupC2.dist_selInv_le_of_rear_family_sup_C2
    ...
    hRbd hRbm' hr' hgSd hgSbd hDm' hmc' hm0' hmstop' hmge' hnumA' hnumK' hsupA' hsupB'
  simpa only [hint] using key

/-! ### The bound as a modulus of the cost of the path of fronts -/

/-- **The bound of the estimate, as a function of the cost of the path of
fronts alone.** -/
def selInvFrontModulus (P1 kh ell L kb kL Pv0 khat dd c : ℝ) : ℝ :=
  SelInvC2Modulus.selInvC2Modulus P1 kh ell L kb kL Pv0 khat (rearKappa2 kh) c
    (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c)

/-- The right-hand side of `dist_selInv_le_of_front_cost_C2` *is* the modulus. -/
theorem bound_eq_front_modulus (P1 kh ell L kb kL Pv0 khat dd c : ℝ) :
    markingC2Bound (2 * P1 * (kh / (1 - kh ^ 2)) * c)
        (flowDefectC1Int ell (kh / (1 - kh ^ 2) * c))
        (flowDefectC2Int ell (kh / (1 - kh ^ 2) * c) (gaugeGrowth2 kh * c)) L kb kL
      + c2ConstVar Pv0
          (costP1 ell khat (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c))
          khat
          (costG1 ell khat (rearKappa2 kh)
            (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c))
          (khat * costG1 ell khat (rearKappa2 kh)
              (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c)
            + rearKappa2 kh * costP1 ell khat
              (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c) ^ 2)
        * (RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c)
    = selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd c := rfl

/-- The modulus is continuous in the cost. -/
theorem continuous_selInvFrontModulus (P1 kh ell L kb kL Pv0 khat dd : ℝ) :
    Continuous fun c : ℝ => selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd c := by
  have h := SelInvC2Modulus.continuous_selInvC2Modulus P1 kh ell L kb kL Pv0 khat
    (rearKappa2 kh)
  have hpair : Continuous fun c : ℝ =>
      (c, RearCostDensity.rearCostConst kh khat (rearKappa2 kh) ell dd * c) := by
    fun_prop
  simpa [Function.comp, selInvFrontModulus] using h.comp hpair

/-- The modulus vanishes at zero cost. -/
theorem selInvFrontModulus_zero (P1 kh ell L kb kL Pv0 khat dd : ℝ) :
    selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd 0 = 0 := by
  unfold selInvFrontModulus
  simpa using SelInvC2Modulus.selInvC2Modulus_zero P1 kh ell L kb kL Pv0 khat (rearKappa2 kh)

/-- **The estimate is a modulus of continuity in the cost of the path of
fronts**: its bound tends to zero as the cost of the path does. -/
theorem tendsto_selInvFrontModulus_zero (P1 kh ell L kb kL Pv0 khat dd : ℝ) :
    Filter.Tendsto (fun c : ℝ => selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd c)
      (nhds 0) (nhds 0) :=
  (continuous_selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd).tendsto' _ _
    (selInvFrontModulus_zero P1 kh ell L kb kL Pv0 khat dd)

/-- **The estimate in `ε`–`δ` form**: for every `ε > 0` there is a bound on the
cost of the path of fronts below which the two marked selected inverses are
within `ε` of one another. -/
theorem exists_cost_bound_of_pos (P1 kh ell L kb kL Pv0 khat dd : ℝ) {eps : ℝ}
    (heps : 0 < eps) :
    ∃ d > 0, ∀ c : ℝ, |c| ≤ d → selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd c ≤ eps := by
  have hcont :=
    (continuous_selInvFrontModulus P1 kh ell L kb kL Pv0 khat dd).continuousAt (x := 0)
  rw [Metric.continuousAt_iff] at hcont
  obtain ⟨d, hd, hball⟩ := hcont eps heps
  refine ⟨d / 2, by positivity, fun c hc => ?_⟩
  have hdist : dist c (0 : ℝ) < d := by
    rw [Real.dist_eq, sub_zero]
    linarith
  have hlt := hball hdist
  rw [Real.dist_eq, selInvFrontModulus_zero, sub_zero] at hlt
  exact le_of_lt (lt_of_abs_lt hlt)

end SelInvFrontCostC2
