...
/-!
# The path-distance bound for the marked selected inverse of the initial curve

`FrontFromPathNormalized.pathDist_le_of_path_normalized` bounds the path
pseudodistance of the selected rears of a normal path, with the initial marked
curve of the bound *assumed* to be the rear track of the initial slice
written in its own arclength (`hstart`).  `SelectedInverseRearOwn.lean` shows
that the **marked selected inverse** of a member of the tube
(`SelectedInverseTube.lean`) is exactly that curve.

This file joins the two: for a normal path of fronts whose initial curve is a
member of the tube, the bound holds with `p'` the value `selInv κ̂ p` of the
selected-inverse map of `SelectedInverseMap.lean` — a curve of the tube, an
oval, whose unit-tangent transform retraces the initial curve.  All the
identifications needed are proved here:

* the arclength period of the path at time `0` is the perimeter of its initial
  marked curve, and the initial slice written in its own arclength is its
  arclength parametrization;
* the tangent-angle lift is irrelevant for the rear track
  (`rearTrack_congr_angle`), so the front data read off the marked curve and
  the canonical front data of the path give the same rear;
* the selected steering angle is unique (`Shadowing.steering_unique`) and so is
  the change of variable to the rear arclength, so the two constructions agree.

Main result: `exists_marked_rearOwn_pathDist`.
-/
...
namespace SelectedInverseRearOwnPath
...
Let `Γ` be a normal path whose initial marked curve `p` is a member of the tube
of curvature pinched by `0 < kmin ≤ K ≤ κ̂ < 1`, and which satisfies the
hypotheses of the normalized bound of
`FrontFromPathNormalized.pathDist_le_of_path_normalized`.  Then the marked
selected inverse `p'` of `p` — a member of the tube, an oval whose unit-tangent
transform retraces `p` — is the initial curve of that bound: the path
pseudodistance from `selInv κ̂ p` to the selected rear at the end of the path is
at most the gauge constant times the cost of `Γ`. -/
theorem exists_marked_rearOwn_pathDist {p q : Data} (Γ : NormalPath p q)
    {c kmin dlt Md MP CK CP : ℝ} {sf : ℝ → ℝ → ℝ}
  ...
  obtain ⟨EF, hEF0, hEFbd, Phi, hPhi0, hbase, hPhi⟩ :=
    FrontFromPathNormalized.pathDist_le_of_path_normalized Γ p' hP0 hkh0 hkh1 hPl hPu hV hA
      ...
      hKdnC3 hFc4 hΘc4 hsfinv hstart
  ...
  exact ⟨dR, hdRpos, hmem', hperimp', hoval', hrange', EF, hEF0, hEFbd, Phi, hPhi0, hbase, hPhi⟩

end SelectedInverseRearOwnPath
