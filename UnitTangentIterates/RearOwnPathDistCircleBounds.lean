...
/-!
# A genuine instance of the path-distance assembly with prescribed constants

`RearOwnPathDistFrameBounds.pathDist_le_of_front_frame_bounds` bounds the path
pseudodistance of the selected rears with the two gauge constants supplied by
the caller rather than produced from the path.  Its hypotheses can all be met at
once, by the same front as in `RearOwnPathDistCircle.lean`: the circle of
curvature `1/2`, at rest.  Its rear does not move, so the tangential component
of the motion vanishes identically and both constants may be taken to be zero.

Main result: `restingCircle_instance_bounds`.
-/
...
namespace RearOwnPathDistCircleBounds

open UniformFrameBounds RearOwnPathDistFrameBounds

/-- **The hypotheses of the path-distance assembly with the rear length free
with prescribed constants are met by a genuine instance.**  The resting circle
of curvature `1/2` produces a front to which
`RearOwnPathDistFrameBounds.pathDist_le_of_front_frame_bounds` applies with both
gauge constants equal to zero — every one of its hypotheses holds for it — and
the bound it gives, the cost of the constant path being zero, is that the
selected rear does not move.  The witnesses are explicit: the rear period `Q = 4π cos(arcsin ½)` is
positive, the gauge parameter starts as the rescaled arclength, and the marked
curve `p'` is the selected rear of the circle. -/
theorem restingCircle_instance_bounds :
  ...
  have hstart : ∀ u, p'.1 u = rearOwn Ff Th de sff 0 (Q * u) := by
    ...
    ring
  -- the tangential component of the motion vanishes, so both constants are zero
  have hzero : frameTangential (fun _ _ : ℝ => (0 : ℂ)) (rearOwnAngle Th de sff)
      = fun _ _ : ℝ => (0 : ℝ) := by
    funext a x; simp [frameTangential]
  have hpz : partialX (fun _ _ : ℝ => (0 : ℝ)) = fun _ _ : ℝ => (0 : ℝ) := by
    funext a x
    show (fderiv ℝ (fun _ : ℝ × ℝ => (0 : ℝ)) (a, x)) (0, 1) = 0
    simp
  have hrL : ∀ t x,
      |partialX (frameTangential (fun _ _ : ℝ => (0 : ℂ)) (rearOwnAngle Th de sff)) t x| ≤ 0 := by
    intro t x; rw [hzero, hpz]; simp
  have hrB : ∀ t x,
      |partialX (partialX (frameTangential (fun _ _ : ℝ => (0 : ℂ))
        (rearOwnAngle Th de sff))) t x| ≤ 0 := by
    intro t x; rw [hzero, hpz, hpz]; simp
  -- the assembly
  ...
  obtain ⟨Phi, hPhi0, -, hPhi⟩ := pathDist_le_of_front_frame_bounds
    ...
    hQd contDiff_const hangC hrL hrB
...
end RearOwnPathDistCircleBounds
