...
`SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2` still asks the
...
Main result: `exists_bounds_dist_selInv_le_modulus_of_path_base_C2`.
-/
...
namespace SelInvPathBoundsBaseC2

open SelInvPathBoundsFundamentalDriftC2
...
`SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2` with the two
constants pinching the perimeter of the slices removed; they are produced in
the conclusion instead. -/
theorem exists_bounds_dist_selInv_le_modulus_of_path_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact ⟨P0, P1, hP0, hPl, hPu,
    SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2 Γ hc hkmin hp hturnp hcq hkminq hq hturnq
      hP0 hkh1 hXC6 hPl hPu hconst hXper hturn hnu hKn0 hKnk hslit hmark⟩

end SelInvPathBoundsBaseC2
