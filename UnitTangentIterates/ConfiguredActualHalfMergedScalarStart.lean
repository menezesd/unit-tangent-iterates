...
namespace ConfiguredActualHalfMergedScalarStart

open ConfiguredCombinedPhysicalDiagonalLargeSeparation
open ConfiguredMarkingAwareMergedEndpointGrowth
...
structure Output (MA NA : ℝ) where
  ...
  large : ExponentialDiagonalLargeSeparation.Output
    E.data (mergedCombinedConversion E.data MA NA
...
theorem exists_output_of_eps
  ...
  obtain ⟨E, direction, Cw, Mend, hCw, hMend, hdir, hwidth, hdM, ⟨L⟩⟩ :=
    exists_actualHalf_widthData_and_mergedOutput_analytic_of_eps
      heps heps10 hMA hNA
...
end ConfiguredActualHalfMergedScalarStart
