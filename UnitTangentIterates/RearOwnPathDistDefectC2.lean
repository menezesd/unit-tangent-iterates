...
`RearOwnPathDistDefect.pathDist_and_defect_le_of_front_family` states the
path-pseudodistance bound for the selected rears of a path of fronts together
with the *position* defect of the gauge marking in which its terminal end is
read.  This file carries the `C²` form of that defect through the same step:
the family of rear tracks is removed from the hypotheses of
`GaugePathRearFamilyDefectC2.pathDist_and_distC2_le_of_rear_family`, being
constructed from the front as `R(t, sf(t,x))`.

Besides the hypotheses of the `C⁰` form it asks for a bound `C₂ t ≤ κ₂·m t` for
the second arclength derivative of the tangential component — supplied by the
geometry in `RearOwnTangentialCostC2.lean` — and for a member `b` of the tube
tracing the terminal slice in its arclength.  The conclusion adds that the curve
read in the marking of the final time is at marked distance at most
`markingC2Bound …` from `b` itself.

Main result: `pathDist_and_distC2_le_of_front_family`.
-/
...
namespace RearOwnPathDistDefectC2
...
together with the `C²` defect of the gauge marking in which it is read.**
...
period, to deviate from the affine marking of the terminal period by at most
`2 L_max κ · cost Γ`, and to read the terminal slice as a marked curve at
distance at most `markingC2Bound …` from that slice itself. -/
theorem pathDist_and_distC2_le_of_front_family {p q : Data} (Γ : NormalPath p q) (p' b : Data)
    {P0 P1 kh : ℝ} {P Qf' : ℝ → ℝ}
  ...
  obtain ⟨Phi, hPhi0, hbase, hone, hflow, hdefect, hdistC2, hPhi⟩ :=
    GaugePathRearFamilyDefectC2.pathDist_and_distC2_le_of_rear_family Γ p' b D
    ...
    (fun t x => norm_rearOwn_tangent t x) hclose hstart hxi0 hxi1bd hCcont hxi2bd hC2cont
    hQmax hcost hcost2 hcq hb hperimb hevb hevd hΘb hkbd hklip
  have hpt : partialTime (rearOwn F Θ δ sf) = Ydot := by
    funext t x
    exact (hasDerivAt_partialTime (hYC.differentiable (by norm_num)) t x).unique (hYd t x)
  refine ⟨Phi, hPhi0, hbase, hone, fun u t => ?_, hdefect, hdistC2, hPhi⟩
  refine (hflow u t).congr_deriv ?_
  rw [hpt, hxiD t (Phi t u)]

/-- **The path pseudodistance of the selected rears from smooth dependence of the
front data, together with the `C²` defect of the gauge marking.**  Same as
...
exactly one rear period, to deviate from the affine marking of the terminal
period by at most `2 L_max κ · cost Γ`, and to read the terminal slice at marked
distance at most `markingC2Bound …` from it. -/
theorem pathDist_and_distC2_le_of_front_smoothDependence {p q : Data} (Γ : NormalPath p q)
    (p' b : Data) {P0 P1 kh : ℝ} {P Qf' : ℝ → ℝ}
  ...
  exact pathDist_and_distC2_le_of_front_family Γ p' b
    ...
    hv1 hxiD hrest hQd hstart hxi0 hxi1bd hCcont hQmax hcost hxi2bd hC2cont hcost2
    hcq hb hperimb hevb hevd hΘb hkbd hklip

end RearOwnPathDistDefectC2
