...
`SelInvMarkingDefectRearFamilyC2.dist_selInv_le_of_rear_family_C2` bounds the
...
Main result: `dist_selInv_le_of_rear_family_sup_C2`.
-/
...
namespace SelInvRearFamilySupC2
...
`SelInvMarkingDefectRearFamilyC2.dist_selInv_le_of_rear_family_C2`, with the
condition on the marking traded for two conditions on the cost density of the
...
unconditional bound for the marked distance of the two selected inverses. -/
theorem dist_selInv_le_of_rear_family_sup_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  obtain ⟨hperimp, hperimq, Phi, hPhi0, hbase, hPhiflow, hmain⟩ :=
    SelInvMarkingDefectRearFamilyC2.dist_selInv_le_of_rear_family_C2 Γ hc hkmin hp hub
      ...
      hr hgSd hgSbd hDm hmc hm0 hmstop hmge hnumA hnumK
  ...
  exact supNorm_le_of_flow_jacobi (xi := frameTangential Ydot
    ...
    Γ.T_pos hellpos hxiC1.continuous (fun s x => hasDerivAt_partialX hxiC1 s x) hpxC1.continuous
