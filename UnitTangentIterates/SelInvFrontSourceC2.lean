...
Main result: `dist_selInv_le_of_front_source_C2`.
-/
...
/-- **The bound on the parameter derivative of the normal speed is the sup
condition of a normal path.**  Where the normal speed of the path is
differentiable in the parameter, with derivative of bounded range, the cost
density dominates that derivative — this is the field `le_m_sup` for `j = 1`,
read pointwise. -/
theorem abs_etaU_le_m {p q : Data} (Γ : NormalPath p q) {etaU : ℝ → ℝ → ℝ}
    (hetaU : ∀ t u, HasDerivAt (Γ.eta t) (etaU t u) u)
    (hbdd : ∀ t, BddAbove (Set.range fun v => |etaU t v|)) (t u : ℝ) :
    |etaU t u| ≤ Γ.m t := by
  have hderiv : iteratedDeriv 1 (Γ.eta t) = etaU t := by
    funext v
    rw [iteratedDeriv_one]
    exact (hetaU t v).deriv
  have h := NormalPathC2IncrementFlow.abs_iteratedDeriv_eta_le Γ t u (j := 1) (by norm_num)
    (by rw [hderiv]; exact hbdd t)
  rwa [hderiv] at h
...
theorem dist_selInv_le_of_front_source_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_front_cost_C2
    ...
    hRbd hRbm hr hgSd hgSbd (fun _ => le_rfl) hnumA hnumK
    (jacobiSourceConst_nonneg hP0) hsmall

end SelInvFrontSourceC2
