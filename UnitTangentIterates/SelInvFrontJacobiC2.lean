...
`SelInvFrontMixedC2.dist_selInv_le_of_front_mixed_C2` still assumes the inverse
Jacobi ODE
...
* `dist_selInv_le_of_front_jacobi_C2` — the estimate with the inverse Jacobi
...
/-- **The chain rule in terms of the partial derivatives.**  For a family
differentiable in the pair, the derivative of `t ↦ f(t, g t)` is
`∂_t f + g'(t) ∂ₓ f`. -/
theorem hasDerivAt_comp_partials {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {f : ℝ → ℝ → E} {g : ℝ → ℝ} {gd t : ℝ}
  ...
  have hc : HasDerivAt (fun r : ℝ => ((r, g r) : ℝ × ℝ)) ((1, gd) : ℝ × ℝ) t := by
    simpa using ((hasDerivAt_id t).prodMk hg)
  have h2 := h1.comp_hasDerivAt t hc
  have hsplit : ((1, gd) : ℝ × ℝ) = ((1, 0) : ℝ × ℝ) + gd • ((0, 1) : ℝ × ℝ) := by
...
/-- **The `C²` comparison of the two marked selected inverses, with the inverse
Jacobi ODE of the family of selected rears proved rather than assumed.**

The hypotheses are those of
`SelInvFrontMixedC2.dist_selInv_le_of_front_mixed_C2` with the steering angle
asked to be jointly `C²` instead of jointly `C¹`, and with the inverse Jacobi
ODE, the normal velocity of the fronts and its link with the normal speed of the
path all removed. -/
theorem dist_selInv_le_of_front_jacobi_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_front_mixed_C2 (etaF := fun t' s => Γ.eta t' (s / P t'))
    ...
    hlink hkappa1 hRbd hRbm hr hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontJacobiC2
