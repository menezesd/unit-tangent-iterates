...
`SelInvPathBoundsBaseC2.exists_bounds_dist_selInv_le_modulus_of_path_base_C2` still
...
Main result: `dist_selInv_le_modulus_of_path_turning_base_C2`.
-/
...
namespace SelInvPathTurningBaseC2
...
`SelInvPathBoundsBaseC2.exists_bounds_dist_selInv_le_modulus_of_path_base_C2` with the
...
threshold `κ̂·L < 4π`. -/
theorem dist_selInv_le_modulus_of_path_turning_base_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact SelInvPathBoundsBaseC2.exists_bounds_dist_selInv_le_modulus_of_path_base_C2 Γ hc hkmin hp hturnp hcq hkminq hq
    hturnq hkh1 hXC6 hconst hXper hturn hnu hKn0 hKnk hslit hmark

end SelInvPathTurningBaseC2
