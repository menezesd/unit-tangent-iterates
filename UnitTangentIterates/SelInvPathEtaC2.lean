...
`SelInvPathCurvBoundC2.dist_selInv_le_modulus_of_path_curv_C2` still asks that
...
Main result: `dist_selInv_le_modulus_of_path_eta_C2`.
-/
...
/-- **The normal speed of a normal path is the normal component of its
velocity.**  Since `∂_tX = η ν` and `ν` is a unit vector, `η = Re(∂_tX conj ν)`. -/
theorem eta_eq_re {p q : Data} (Γ : NormalPath p q)
    (hX : Differentiable ℝ (uncurry Γ.X)) (t u : ℝ) :
    Γ.eta t u = (partialTime Γ.X t u * (starRingEnd ℂ) (Γ.nu t u)).re := by
  have h1 : partialTime Γ.X t u = (Γ.eta t u : ℂ) * Γ.nu t u :=
    (hasDerivAt_partialTime hX t u).unique (Γ.hasDerivAt_time t u)
  have hn : Γ.nu t u * (starRingEnd ℂ) (Γ.nu t u) = 1 := by
    rw [Complex.mul_conj]
    norm_cast
    rw [Complex.normSq_eq_norm_sq, Γ.norm_nu t u]
    norm_num
  rw [h1, mul_assoc, hn, mul_one, Complex.ofReal_re]

/-- **The normal speed of a smooth normal path is smooth in the parameter.**
For a jointly `C^{n+2}` moving curve whose unit normal is the standard one of
its slices, the normal speed is `Cⁿ` in the parameter. -/
theorem contDiff_eta_of_path {p q : Data} (Γ : NormalPath p q) {n : ℕ}
    (hXC : ContDiff ℝ ((n + 1 : ℕ)) (uncurry Γ.X))
    (hnu : ∀ t u, Γ.nu t u = Complex.I * ((pathVel Γ.X) t u / (P t : ℂ))) (t : ℝ) :
    ContDiff ℝ (n : ℕ) (Γ.eta t) := by
  have hX1 : Differentiable ℝ (uncurry Γ.X) := hXC.differentiable (by norm_num)
  have heq : Γ.eta t = fun u =>
      (partialTime Γ.X t u *
        (starRingEnd ℂ) (Complex.I * (pathVel Γ.X t u / (P t : ℂ)))).re := by
    funext u; rw [eta_eq_re Γ hX1 t u, hnu t u]
  have hslice : ContDiff ℝ (n : ℕ) (fun u : ℝ => ((t, u) : ℝ × ℝ)) :=
    contDiff_const.prodMk contDiff_id
  have h1 : ContDiff ℝ (n : ℕ) (fun u : ℝ => partialTime Γ.X t u) :=
    (contDiff_partialTime_self hXC).comp hslice
  have h2 : ContDiff ℝ (n : ℕ) (fun u : ℝ => pathVel Γ.X t u) :=
    (contDiff_partialArc_self hXC).comp hslice
  rw [heq]
  refine Complex.reCLM.contDiff.comp (h1.mul ?_)
  exact Complex.conjCLE.contDiff.comp (contDiff_const.mul (h2.div_const _))
...
`SelInvPathCurvBoundC2.dist_selInv_le_modulus_of_path_curv_C2` with the `C¹`
...
normal. -/
theorem dist_selInv_le_modulus_of_path_eta_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    ∃ dn δ : ℝ → ℝ → ℝ,
      ...
      dist (SelectedInverseMap.selInv kh q) (SelectedInverseMap.selInv kh p)
        ≤ selInvFrontModulus P1 kh (perim (SelectedInverseMap.selInv kh p))
            ...
            (cost Γ) := by
  refine dist_selInv_le_modulus_of_path_curv_C2 Γ hc hkmin hp hturnp hcq hkminq hq hturnq
