...
/-!
# A genuine instance of the path-distance assembly

`RearOwnPathDistFrame.pathDist_le_of_front_frame` bounds the path pseudodistance
of the selected rears of a path of fronts by hypotheses which are all on the
front data.  The hypotheses are many, and one of them — that the rear arclength
period does not change along the path — is forced
(`GaugePeriodRigidity.lean`); so it is worth checking that they can all be met
at once.

They can: the circle of curvature `1/2`, at rest, is a front with tangent angle
`Θ(s) = s/2`, selected steering angle `arcsin ½`, period `4π` and change of
variable `x ↦ x sec(arcsin ½)`; its selected rear is the circle of radius
`|−2i − e^{−i arcsin ½}|`, and the constant path of that circle satisfies every
hypothesis of the assembly.  The conclusion obtained is that the selected rear
does not move: the path pseudodistance between the two marked curves it produces
is zero.

Main result: `restingCircle_instance`.
-/
...
namespace RearOwnPathDistCircle

open UniformFrameBounds RearOwnPathDistFrame

/-- **The hypotheses of the path-distance assembly are met by a genuine
instance.**  The resting circle of curvature `1/2` produces a front to which
`RearOwnPathDistFrame.pathDist_le_of_front_frame` applies — every one of its
hypotheses, geometric, regular and structural, holds for it — and the bound it
gives, the cost of the constant path being zero, is that the selected rear does
not move.  The witnesses are explicit: the rear period `Q = 4π cos(arcsin ½)` is
positive, the gauge parameter starts as the rescaled arclength, and the marked
curve `p'` is the selected rear of the circle. -/
theorem restingCircle_instance :
  ...
  obtain ⟨rL, rB, Phi, hPhi0, -, hPhi⟩ := pathDist_le_of_front_frame (P0 := 4 * Real.pi)
    (P1 := 4 * Real.pi) (kh := 1 / 2) (Q := Q) (P := fun _ => 4 * Real.pi)
    ...
    hQpos hQconst contDiff_const hangC (fun a x => rfl) (fun a x => rfl)
