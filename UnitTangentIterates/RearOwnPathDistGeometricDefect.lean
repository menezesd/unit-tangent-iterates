...
off the geometry, together with the defect of its gauge marking
...
for the same marking `Φ`, the three facts the comparison of the two marked
selected inverses needs — `Φ` fixes the base point, reads exactly one rear
...
Main result: `pathDist_and_defect_le_of_front_normalVelocity`.
-/
...
namespace RearOwnPathDistGeometricDefect
...
theorem pathDist_and_defect_le_of_front_normalVelocity {p q : Data} (Γ : NormalPath p q)
    (p' : Data) {P0 P1 kh EF : ℝ} {P Qf' : ℝ → ℝ}
  ...
  exact RearOwnPathDistFrameBoundsDefect.pathDist_and_defect_le_of_front_frame_bounds Γ p'
    ...
    (fun t => le_of_eq (mul_comm _ _))

end RearOwnPathDistGeometricDefect
