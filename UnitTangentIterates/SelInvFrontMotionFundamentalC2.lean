...
`SelInvFrontStripFundamentalC2.dist_selInv_le_of_front_strip_fundamental_C2` still asks the caller for
...
* `dist_selInv_le_of_front_motion_fundamental_C2` — the estimate with `alphaT`, `kT` and
...
namespace SelInvFrontMotionFundamentalC2
...
theorem dist_selInv_le_of_front_motion_fundamental_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact SelInvFrontStripFundamentalC2.dist_selInv_le_of_front_strip_fundamental_C2
    ...
    hRbd hRbm hr hxi0 hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontMotionFundamentalC2
