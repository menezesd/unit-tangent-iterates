...
/-- **The normal path with slices of variable speed produced by a gauge marking**,
from the packaged data. -/
theorem exists_variableSpeed_normalPath_of_data {a b : Data} {P0 P1 khat G1 Cg T : ℝ}
    {m : ℝ → ℝ} (D : GaugeMarkedData a b P0 P1 khat G1 Cg T m) :
    ∃ Γ : NormalPath a b, Γ.T = T ∧ Γ.m = m ∧ cost Γ = (∫ t in (0 : ℝ)..T, m t) ∧
      IsVariableSpeedNormalPath P0 P1 khat G1 Cg Γ := by
  obtain ⟨Γ, hΓT, -, hΓm, hΓcost, hΓvar⟩ :=
    exists_variableSpeed_normalPath_of_gauge_marking (a := a) (b := b)
