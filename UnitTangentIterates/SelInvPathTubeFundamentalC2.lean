...
`SelInvPathTurningFundamentalC2.dist_selInv_le_modulus_of_path_turning_fundamental_C2` still asks the
...
Main result: `dist_selInv_le_modulus_of_path_tube_fundamental_C2`.
-/
...
namespace SelInvPathTubeFundamentalC2
...
`SelInvPathTurningFundamentalC2.dist_selInv_le_modulus_of_path_turning_fundamental_C2` with the two
...
theorem dist_selInv_le_modulus_of_path_tube_fundamental_C2 {p q : Data} (Γ : NormalPath p q)
    {kminP : ℝ}
  ...
  have hperimq : 0 < perim q := by
    ...
    rw [← this]
    exact hPpos Γ.T
  exact SelInvPathTurningFundamentalC2.dist_selInv_le_modulus_of_path_turning_fundamental_C2 Γ hperimp hkminP hp hperimq hkminP hq
    hkh1 hXC6 hconst hXper hnu hkminP hKnmin hKnk hshort hslit hmark

end SelInvPathTubeFundamentalC2
