...
/-!
# The path pseudodistance from the front curve alone, together with the C² defect of
its gauge marking
...
base point, reads exactly one rear period, deviates from the affine marking of
the terminal period by at most `2 P₁ κ̂/(1 − κ̂²) · cost Γ`, and leaves the
terminal marked curve within `markingC2Bound` of the marked reference curve `b`
in the `C²` metric — under the single extra hypothesis that the base drift of the
front vanishes.

Main result: `pathDist_and_distC2_le_of_front_curve`.
-/
...
namespace RearOwnPathDistFrontOnlyDefectC2

open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  RearOwnHigherRegularity MarkingDeviationC2 MarkingFlowDefectC2 RearOwnTangentialCostC2

/-- **The path pseudodistance of the selected rears from the front curve alone,
together with the C² defect of its gauge marking.**
`RearOwnPathDistFrontOnly.pathDist_le_of_front_curve` with the extra
conclusions about the marking. -/
theorem pathDist_and_distC2_le_of_front_curve {p q : Data} (Γ : NormalPath p q) (p' b : Data)
  ...
  exact RearOwnPathDistSmoothDefectC2.pathDist_and_distC2_le_of_front_data Γ p' b
...
end RearOwnPathDistFrontOnlyDefectC2
