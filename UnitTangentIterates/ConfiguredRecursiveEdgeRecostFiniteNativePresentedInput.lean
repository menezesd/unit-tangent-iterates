...
/-! # Native presented inputs for finite-major intrinsic successors -/

noncomputable section

open Function Set MarkedSpace PathMetric

namespace ConfiguredRecursiveEdgeRecostFiniteNativePresentedInput
...
/-- Source-specific terminal facts, with no reference to the legacy infinite
gauge major. -/
structure BoundaryFacts
    (I : ConfiguredRecursiveEdgeRecostFiniteIntrinsicStepAssembly.InputData
      J H budget L)
    (n : ℕ) (bound : ℝ) where
  recursiveFacts : Input.RecursiveFacts (I.analytic n)
...
variable {I : ConfiguredRecursiveEdgeRecostFiniteIntrinsicStepAssembly.InputData
    J H budget L} {n : ℕ} {bound : ℝ}
...
noncomputable def geometry (B : BoundaryFacts I n bound) :
  ...
  Classical.choice
    (RecursiveAnalyticSuccessor.exists_presentedTerminalGeometry_of_spatial
      B.recursive (I.step.nextApplied n)
...
end ConfiguredRecursiveEdgeRecostFiniteNativePresentedInput
