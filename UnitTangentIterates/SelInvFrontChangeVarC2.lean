...
`SelInvFrontRegularityC2.dist_selInv_le_of_front_regularity_C2` still carries
...
`dist_selInv_le_of_front_changevar_C2`.
-/
...
namespace SelInvFrontChangeVarC2
...
/-- **The canonical change of variable from the rear arclength back to the
front arclength**: the inverse of `s ↦ rearArclength (δ t) s`. -/
def rearArclengthInv (δ : ℝ → ℝ → ℝ) : ℝ → ℝ → ℝ :=
  fun t => Function.invFun (rearArclength (δ t))

/-- **On the selected strip the canonical change of variable does invert the
rear arclength.** -/
theorem rearArclength_rearArclengthInv {kap : ℝ}
    (hkap0 : 0 ≤ kap) (hkap1 : kap < 1)
    (hdc : ∀ t, Continuous (δ t)) (hd0 : ∀ t s, 0 ≤ δ t s)
    (hd1 : ∀ t s, δ t s ≤ Real.arcsin kap) (t x : ℝ) :
    rearArclength (δ t) (rearArclengthInv δ t x) = x := by
  obtain ⟨g, hg⟩ := exists_inverse_rearArclength hkap0 hkap1 (hdc t) (hd0 t) (hd1 t)
  exact Function.invFun_eq ⟨g x, hg x⟩

theorem dist_selInv_le_of_front_changevar_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    (hRbd : ∀ t x,
      |frameTangential (rearOwnVelocity Γ.X V A P δ (rearArclengthInv δ)) (rearOwnAngle (angleOfPath V A P) δ (rearArclengthInv δ)) t x| ≤ Rb t)
  ...
  exact dist_selInv_le_of_front_regularity_C2
