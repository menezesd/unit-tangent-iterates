...
`SelInvPathEtaC2.dist_selInv_le_modulus_of_path_eta_C2` still carries the
...
Main result: `dist_selInv_le_modulus_of_path_perim_C2`.
-/
...
/-- **The arclength period of the slices of a path**: the speed of the moving
curve at the marked point, which for a constant-speed slice is its perimeter. -/
def pathPerim (X : ℝ → ℝ → ℂ) : ℝ → ℝ := fun t => ‖pathVel X t 0‖
...
are those of `SelInvPathEtaC2.dist_selInv_le_modulus_of_path_eta_C2` with the
...
theorem dist_selInv_le_modulus_of_path_perim_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    ∃ dn δ : ℝ → ℝ → ℝ,
      ...
      dist (SelectedInverseMap.selInv kh q) (SelectedInverseMap.selInv kh p)
        ≤ selInvFrontModulus P1 kh (perim (SelectedInverseMap.selInv kh p))
            ...
            (cost Γ) := by
  exact dist_selInv_le_modulus_of_path_eta_C2 Γ hc hkmin hp hturnp hcq hkminq hq hturnq
    hP0 hkh1 hXC6 hPl hPu (fun t => rfl) hconst hXper hturn hnu hKn0 hKnk hslit hmark

end SelInvPathPerimC2
