...
namespace ConfiguredMarkingAwareCorrelatedDiagonalDirectAssembly
...
open ConfiguredMarkingAwarePhysicalProducerFromCap
open FiniteSmoothRearFamilyMarkingAwareDirectCapstone
...
theorem conclude_of_physicalProducer
  ...
  apply FiniteSmoothRearFamilyMarkingAwareDirectCapstone.exists_paperFacingOutput
    F RB hbase (ConfiguredAlignedQGeometry.acceleration_le A hQ)
      hpartial hradius physical A.input.kh_nonneg A.input.kh_lt_one
      hH0 (half_pos hchord) hH0
      hdirection
    ...
    Nonempty (
      Σ O : TriangularMarkedPathSchemeVariableTerminal.LimitOutput
        ...
        PaperFacingVariableTerminalOutput.Output O direction Cw
          ((shift D L.N).Hs 0)) := by
  let physical :=
    ConfiguredMarkingAwarePhysicalProducerFromCap.physicalProducer_of_capFamily
...
/-- Configured diagonal assembly over the invariant-indexed reachable core. -/
theorem sliced_conclude_of_physicalProducer
    {D : ConstructedConfiguredSequenceWeighted.Data}
    {conversion diagonal : ℕ → ℝ} {Cw : ℝ}
    (L : Output D conversion diagonal Cw)
    {Qmodel : ℕ → Data} {kh0 C0 K : ℝ} {d : ℕ → ℝ}
    ...
    (physical : SlicedPhysicalProducer F kh0 ((shift D L.N).Hs 0)
      (chordBase (shift D L.N).model / 2))
    (hconversion0 : ∀ n, 0 ≤ conversion n)
    (hdiagonal0 : ∀ n, 0 ≤ diagonal n)
    (hpathConversion : ∀ n,
    ...
    Nonempty (Σ O : TriangularMarkedPathSchemeVariableTerminal.LimitOutput
      (ConfiguredGaugeFirstPhysicalSequence.alignedQ A.input hQ)
      (fun n k ↦ F.columns k n) (rowError (shiftSequence diagonal L.N))
  ...
  have hradius : ∀ n,
    ...
    exact mul_le_mul_of_nonneg_right (hpathConversion n)
      (ShadowingTails.tail_nonneg (F.defect.nonnegative n) 0)
  apply FiniteSmoothRearFamilyMarkingAwareDirectCapstone.sliced_exists_paperFacingOutput
    F RB hbase (ConfiguredAlignedQGeometry.acceleration_le A hQ)
      hpartial hradius physical A.input.kh_nonneg A.input.kh_lt_one
      hH0 (half_pos hchord) hH0 hdirection
  · exact ConfiguredAlignedQGeometry.bounded_range A hQ 0
  · rw [ConfiguredAlignedQGeometry.width_zero_eq_model A hQ]
    exact hmodelWidth
  · rw [ConfiguredAlignedQGeometry.totalLength_eq A hQ 0]
  · intro O
    have hshadow : PaperFacingVariableTerminalOutput.shadowSize O ≤
      ...
      change NormalPathC2IncrementVariableSpeed.c2ConstVar
          (P0 0) (P1 0) (khat 0) (G1 0) (Cg 0) *
          ShadowingTails.tail (rowError (shiftSequence diagonal L.N) 0) 0 ≤ _
      exact mul_le_mul_of_nonneg_right (hpathConversion 0)
        (ShadowingTails.tail_nonneg (fun k ↦ F.defect.nonnegative 0 k) 0)
    have hleft : Cw + 2 * PaperFacingVariableTerminalOutput.shadowSize O ≤
        Cw + 2 * rowRadius (shiftSequence conversion L.N)
          (shiftSequence diagonal L.N) 0 := by linarith
    have hright :
...
theorem sliced_conclude
    {D : ConstructedConfiguredSequenceWeighted.Data}
    {pathConversion endpointConversion diagonal : ℕ → ℝ} {Cw : ℝ}
    (L : Output D (ConfiguredGaugeEndpointLinearRadius.combinedConversion
      pathConversion endpointConversion) diagonal Cw)
    {Qmodel : ℕ → Data} {kh0 C0 K : ℝ} {d : ℕ → ℝ}
    (A : ConfiguredCanonicalPairSource.Output
      (shift D L.N) Qmodel kh0 C0 K d)
    ...
    Nonempty (Σ O : TriangularMarkedPathSchemeVariableTerminal.LimitOutput
      ...
      PaperFacingVariableTerminalOutput.Output O direction Cw
        ((shift D L.N).Hs 0)) := by
  let physical := slicedPhysicalProducer_of_capFamily L A hQ F R hpath0 hendpoint0
      hdiagonal0 hpathConversion hkh hbasePhysical
  exact sliced_conclude_of_physicalProducer L A hQ F physical
    (ConfiguredGaugeEndpointLinearRadius.combinedConversion_nonneg hpath0 hendpoint0)
    hdiagonal0
    (fun n ↦ (hpathConversion n).trans
      (le_add_of_nonneg_right (hendpoint0 (L.N + n))))
    hdirection hmodelWidth

end ConfiguredMarkingAwareCorrelatedDiagonalDirectAssembly
