...
period, and deviates from the affine marking of the terminal period by at most
`2 P₁ κ̂/(1 − κ̂²) · cost Γ` — under the geometric hypothesis that the path does
not move at its marked point.

Main result: `exists_marked_rearOwn_pathDist_and_defect`.
-/
...
together with the defect of its gauge marking.**
`SelectedInverseRearOwnPath.exists_marked_rearOwn_pathDist` with the three extra
conclusions about the marking. -/
theorem exists_marked_rearOwn_pathDist_and_defect {p q : Data} (Γ : NormalPath p q)
