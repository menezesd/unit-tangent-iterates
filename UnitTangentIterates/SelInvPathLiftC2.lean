...
`SelInvPathEmbeddedC2.exists_steering_dist_selInv_le_of_path_embedded_C2` still
...
Main result: `dist_selInv_le_of_path_lift_C2`, in which the two constants of the
marking bound are the explicit ones above.
-/
...
of
`SelInvPathEmbeddedC2.exists_steering_dist_selInv_le_of_path_embedded_C2` with
...
`2κ̂/√(1−κ̂²)³`. -/
theorem dist_selInv_le_of_path_lift_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact exists_steering_dist_selInv_le_of_path_embedded_C2
