...
/-!
# The `C²` estimate with the tube membership produced, at given perimeter bounds

`SelInvPathTurningBaseUniformC2.dist_selInv_le_modulus_of_path_turning_base_uniform_C2` still asks the
...
This is the base-point form — no hypothesis on the tangential drift of the
selected rears is left — in its *uniform* variant: the two bounds
`P₀ ≤ P(t) ≤ P₁` pinching the perimeter of the slices are supplied by the
caller instead of being produced in the conclusion, so that the modulus is
written with the caller's own constants and one modulus serves a whole family
of paths.

Main result: `dist_selInv_le_modulus_of_path_tube_base_uniform_C2`.
-/
...
namespace SelInvPathTubeBaseUniformC2
...
membership of the two ends produced and the perimeter bounds supplied.**  The
hypotheses are those of
`SelInvPathTurningBaseUniformC2.dist_selInv_le_modulus_of_path_turning_base_uniform_C2` with the two
...
theorem dist_selInv_le_modulus_of_path_tube_base_uniform_C2 {p q : Data} (Γ : NormalPath p q)
    {kminP P0 P1 : ℝ}
  ...
  have hperimq : 0 < perim q := by
    ...
    rw [← this]
    exact hPpos Γ.T
  exact SelInvPathTurningBaseUniformC2.dist_selInv_le_modulus_of_path_turning_base_uniform_C2 Γ
    hperimp hkminP hp hperimq hkminP hq hkh1 hP0 hPl hPu hXC6 hconst hXper hnu hkminP hKnmin
    hKnk hshort hslit hmark

end SelInvPathTubeBaseUniformC2
