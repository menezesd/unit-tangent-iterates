...
/-!
# Final paper assembly from approximate pullback paths

This module replaces the legacy family of exact controlled stages by the
paper's actual approximate normal-path input.  The marked matching defect is
allowed to grow by a factor `K` at every selected-rear pullback.  Its
summability is derived from the sharp threshold

`K * exp (-(beta * deltaStep)) < 1`.
-/

noncomputable section

open Function Filter Topology Set MarkedSpace PathMetric PathMetric.NormalPath
open NormalPathC2IncrementVariableSpeed

namespace UnconditionalAssembly

namespace ApproximatePaperAssemblyResidual

open PaperFaithfulAssemblyRemainder
open PathMetric.WeightedMarkedDefectThreshold
open PathMetric.WeightedRecursiveDefect

/-- The zeroth weighted pullback tail appearing in the final Hausdorff and
perimeter estimates. -/
def shadowError
    (K P0 P1 khat G1 Cg Cm L kstar kd beta : ℝ)
    (Hs : ℕ → ℝ) : ℝ :=
  ShadowingTails.tail
    (fun k => c2ConstVar P0 P1 khat G1 Cg *
      (K ^ k * canonicalMarkedDefect Cm L kstar kd beta Hs k)) 0

/-- **Paper-faithful closing from approximate weighted pullback paths.**

All limit, inverse-orbit, strict-oval, forward unit-tangent, Hausdorff, and
noncircularity conclusions are derived.  The remaining hypotheses are the
geometric inputs not supplied by metric completeness: approximate transport
and matching paths, closed-tube invariance, the physical rear reconstruction
of pullback limits, exact front/selected-rear compatibility, and the final
width gap. -/
theorem conclude_of_approx_weighted_markedDefect
    {kappas : ℕ → ℝ → ℝ} {Hs theta0 eps : ℕ → ℝ}
    {B T : Data → Data} {Q : ℕ → Data}
    {K P0 P1 khat G1 Cg Cm L kstar kd H0 deltaStep beta c dlt Cw Csh : ℝ}
    {dir : ℂ}
    ...
    (hmap : ∀ (p q : Data) (Gamma : NormalPath p q),
      ...
      ∀ eta : ℝ, 0 < eta → ∃ Delta : NormalPath (B p) (B q),
    ...
    (hdefect : ∀ n : ℕ, ∀ eta : ℝ, 0 < eta →
      ∃ Lambda : NormalPath (Q n) (B (Q (n + 1))),
        cost Lambda ≤ canonicalMarkedDefect Cm L kstar kd beta Hs n + eta ∧
        IsVariableSpeedNormalPath P0 P1 khat G1 Cg Lambda)
    (hC : 0 ≤ c2ConstVar P0 P1 khat G1 Cg)
    (residual : ClosedTubeInvarianceResidual B Q c dlt)
    (hBcont : Continuous B)
    (physical : PhysicalRearLimitComponentFamily
      (fun n k => TubePullbackLimit.pullback B Q n k))
    (hc : 0 < c) (hdlt : 0 < dlt)
    (hQfront : ∀ n, ev (Q n) =
    ...
    (hgap : Cw + 2 * (Csh *
        shadowError K P0 P1 khat G1 Cg Cm L kstar kd beta Hs) <
      (2 * Hs 0 - Csh *
        shadowError K P0 P1 khat G1 Cg Cm L kstar kd beta Hs) / Real.pi) :
    ∃ (X : ℕ → ℝ → ℂ) (LX : ℝ),
  ...
  let E : ℝ := shadowError K P0 P1 khat G1 Cg Cm L kstar kd beta Hs
  ...
  have hdist0 : dist (Q 0) (X 0) ≤ E := by
    simpa [E, shadowError, d] using hdist 0
  ...
  have hshadow : ∃ (Y : ℕ → ℝ → ℂ) (LY : ℝ),
      ...
      Metric.hausdorffDist (range (Y 0))
        (range (TwoCapPairsAssembly.front (kappas 0) (theta0 0) (Hs 0))) ≤
          Csh * E ∧
      2 * Hs 0 - Csh * E ≤ LY :=
    shadowingOrbit_of_markedLimit hc residual.model_mem hXmem hQfront hinv hTB
      hTev hoval hclose hperim hQperim
...
end ApproximatePaperAssemblyResidual
