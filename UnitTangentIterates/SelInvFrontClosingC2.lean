...
`SelInvFrontChangeVarC2.dist_selInv_le_of_front_changevar_C2` still asks that
...
Main result: `dist_selInv_le_of_front_closing_C2`.
-/
...
namespace SelInvFrontClosingC2
...
`SelInvFrontChangeVarC2.dist_selInv_le_of_front_changevar_C2` with the
periodicity of the normal velocity of the rears removed. -/
theorem dist_selInv_le_of_front_closing_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  have hper : ∀ t, Function.Periodic
    ...
    exact (GaugeClosingRelations.closing_relations hYC hYx hYtframe htau0 htauper hclose
      hQd t x).2
  exact dist_selInv_le_of_front_changevar_C2
