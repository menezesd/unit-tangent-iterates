...
`SelectedInverseRearOwnPath.exists_marked_rearOwn_pathDist` identifies the
initial curve of the bound with the marked selected inverse `selInv κ̂ p`.  This
file restates it with the extra conclusions about the gauge marking `Φ` in which
the pseudodistance is read — `Φ` fixes the base point, reads exactly one rear
period, deviates from the affine marking of the terminal period by at most
`2 P₁ κ̂/(1 − κ̂²) · cost Γ`, and leaves the terminal marked curve within
`markingC2Bound` of the marked reference curve `b` in the `C²` metric — under the
geometric hypothesis that the path does not move at its marked point.

Main result: `exists_marked_rearOwn_pathDist_and_distC2`.
-/
...
namespace SelectedInverseRearOwnPathDefectC2
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  FrontFromPath SelectedInverseRearOwn MarkingDeviationC2 MarkingFlowDefectC2
...
together with the C² defect of its gauge marking.**
`SelectedInverseRearOwnPath.exists_marked_rearOwn_pathDist` with the extra
conclusions about the marking. -/
theorem exists_marked_rearOwn_pathDist_and_distC2 {p q : Data} (Γ : NormalPath p q) (b : Data)
    ...
    (hevb : ∀ x, ev b x = rearOwn (frontOfPath Γ.X P) (angleOfPath V A P) δ sf Γ.T x)
      ...
      ∃ EF : ℝ, 0 ≤ EF ∧
        ...
        ∃ Phi : ℝ → ℝ → ℝ,
          ...
          (∀ u t, HasDerivAt (fun r => Phi r u)
            (-frameTangential (partialTime (rearOwn (frontOfPath Γ.X P) (angleOfPath V A P) δ sf))
              (rearOwnAngle (angleOfPath V A P) δ sf) t
              (Phi t u)) t) ∧
          (∀ u, |Phi Γ.T u - rearArclength (δ Γ.T) (P Γ.T) * u|
            ≤ 2 * P1 * (kh / (1 - kh ^ 2)) * cost Γ) ∧
          (∀ q' : Data, (∀ u, q'.1 u = rearOwn (frontOfPath Γ.X P) (angleOfPath V A P) δ sf Γ.T (Phi Γ.T u)) →
  ...
  obtain ⟨EF, hEF0, hEFbd, Phi, hPhi0, hbase, hone, hflow, hdefect, hdistC2, hPhi⟩ :=
    FrontFromPathNormalizedDefectC2.pathDist_and_distC2_le_of_path_normalized Γ p' b
      ...
      hKdnC3 hFc4 hΘc4 hsfinv hstart hmark
  ...
  exact ⟨dR, hdRpos, hmem', hperimp', hoval', hrange', EF, hEF0, hEFbd, Phi, hPhi0, hbase,
    hone, hflow, hdefect, hdistC2, hPhi⟩

end SelectedInverseRearOwnPathDefectC2
