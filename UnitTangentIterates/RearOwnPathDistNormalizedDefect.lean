...
/-!
# The path pseudodistance in the normalized parameter, together with the defect
...
fixes the base point, reads exactly one rear period, and deviates from the affine
marking of the terminal period by at most `2 P₁ κ̂/(1 − κ̂²) · cost Γ`.  The extra
...
Main results: `pathDist_and_defect_le_of_front_normalized`,
`exists_sf_pathDist_and_defect_le_of_front_normalized`.
-/
...
namespace RearOwnPathDistNormalizedDefect
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  RearOwnPathDistSlices RearOwnPathDistNormalized
...
`RearOwnPathDistNormalized.pathDist_le_of_front_normalized` with the three extra
conclusions about the marking. -/
theorem pathDist_and_defect_le_of_front_normalized {p q : Data} (Γ : NormalPath p q) (p' : Data)
  ...
  obtain ⟨Phi, hPhi0, hbase, hone, hdefect, hPhi⟩ :=
    RearOwnPathDistFrontOnlyDefect.pathDist_and_defect_le_of_front_curve Γ p'
      (Qf' := Qf') (EF := EF)
  ...
  exact ⟨EF, hEF0, hEF, Phi, hPhi0, hbase, hone, hdefect, hPhi⟩
...
theorem exists_sf_pathDist_and_defect_le_of_front_normalized {p q : Data} (Γ : NormalPath p q)
    {Md MP CK CP : ℝ}
    (hP0 : 0 < P0) (hkh0 : 0 ≤ kh) (hkh1 : kh < 1)
    (hPl : ∀ t, P0 ≤ P t) (hPu : ∀ t, P t ≤ P1)
    (hdelta : ∀ t s, δ t s = dn t (s / P t)) (hKeq : ∀ t s, K t s = Kn t (s / P t))
  ...
  refine ⟨sf, hsf, ?_⟩
  intro p' hstart
  exact pathDist_and_defect_le_of_front_normalized Γ p' (Md := Md) (MP := MP) (CK := CK) (CP := CP)
...
end RearOwnPathDistNormalizedDefect
