...
`SelInvPathPerimFundamentalDriftC2.dist_selInv_le_modulus_of_path_perim_fundamental_drift_C2` asks the caller for
...
Main result: `dist_selInv_le_modulus_of_path_gauge_fundamental_drift_C2`.
-/
...
namespace SelInvPathGaugeFundamentalDriftC2

open SelInvPathGaugeFundamentalC2
...
`SelInvPathPerimFundamentalDriftC2.dist_selInv_le_modulus_of_path_perim_fundamental_drift_C2` with the constant
`Pv0` and the two numerical conditions it had to satisfy replaced by the
explicit `pathPv0`. -/
theorem dist_selInv_le_modulus_of_path_gauge_fundamental_drift_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  obtain ⟨dn, δ, hdnper, hstrip, hsol, hdel, hmain⟩ :=
    SelInvPathPerimFundamentalDriftC2.dist_selInv_le_modulus_of_path_perim_fundamental_drift_C2 Γ hc hkmin hp hturnp hcq hkminq hq hturnq
      hP0 hkh1 hXC6 hPl hPu hconst hXper hturn hnu hKn0 hKnk hslit hmark
  refine ⟨dn, δ, hdnper, hstrip, hsol, hdel,
    fun khat hkappa1 hxi0 hsmall => ?_⟩
  ...
  have hr : 0 ≤ P1 * (kh / (1 - kh ^ 2)) :=
    mul_nonneg (le_trans (le_trans hP0.le (hPl 0)) (hPu 0)) (div_nonneg hkh0 hsq.le)
  exact hmain (pathPv0 kh P0 khat (P1 * (kh / (1 - kh ^ 2)))) khat hkappa1 hxi0
    (pathPv0_numA hP0 hkh0 hkhat hr) (pathPv0_numK hP0 hkh0 hkhat hr) hsmall

end SelInvPathGaugeFundamentalDriftC2
