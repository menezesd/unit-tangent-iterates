...
/-!
# The Jacobi path bound for the selected inverse, without a curvature floor

`SelectedInverseRearOwnPathDefect.exists_marked_rearOwn_pathDist_and_defect` is
the `hmap` machinery of the closing chain: it bounds `pathDist (selInv κ̂ p) q'`
by a Jacobi constant times `cost Γ`.  It took `0 < kmin`, which §47 of the
session log shows the main theorem cannot supply.

Its three uses of the floor are all now replaceable:

* `exists_marked_rearOwn` → `exists_marked_rearOwn_floor_free` (§82);
* `selInv_eq`, which carries a floor, → `eq_selInv_of_isMarkedSelectedInverse`,
  which never needed one;
* the rear tube witness `kmin/√(1−kmin²)` → `0`.

Nothing in the Jacobi estimate itself touches the floor.
-/
...
namespace SelectedInverseRearOwnPathDefect
...
theorem exists_marked_rearOwn_pathDist_and_defect_floor_free {p q : Data} (Γ : NormalPath p q)
    {c kmin dlt Md MP CK CP : ℝ} {sf : ℝ → ℝ → ℝ}
    (hc : 0 < c) (hp : IsTubeMember c 0 dlt p)
    ...
    ∃ dR : ℝ, 0 < dR ∧
      IsTubeMember (perim (SelectedInverseMap.selInv kh p))
        0 dR (SelectedInverseMap.selInv kh p) ∧
  ...
  obtain ⟨p', Θ', K', dl, sf', dR, hX', hΘ', hKlow', hKhigh', hdper', hdmem', hode', hsfinv',
    ...
    SelectedInverseRearOwn.exists_marked_rearOwn_floor_free hc hkh1 hp hub hinjR
  ...
  obtain ⟨EF, hEF0, hEFbd, Phi, hPhi0, hbase, hone, hdefect, hPhi⟩ :=
    FrontFromPathNormalizedDefect.pathDist_and_defect_le_of_path_normalized Γ p'
  ...
  have hp'eq : p' = SelectedInverseMap.selInv kh p :=
    SelectedInverseMap.eq_selInv_of_isMarkedSelectedInverse hc hkh0 hkh1 hp
      ⟨⟨perim p', 0, dR, hmem'⟩,
...
end SelectedInverseRearOwnPathDefect
