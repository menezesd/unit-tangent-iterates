...
/-!
# The `C²` estimate with the turning numbers discharged, at given perimeter bounds

`SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2` still
...
This is the base-point form — the vanishing of the tangential drift of the
selected rears at the marked point is discharged from the marked point of the
path being at rest, `∀ t, Γ.eta t 0 = 0` — in its *uniform* variant: the two
bounds `P₀ ≤ P(t) ≤ P₁` pinching the perimeter of the slices are supplied by
the caller instead of being produced in the conclusion, so that the constants
of the modulus are the caller's own and one modulus serves a whole family of
paths.

Main result: `dist_selInv_le_modulus_of_path_turning_base_uniform_C2`.
-/
...
namespace SelInvPathTurningBaseUniformC2

open SelInvPathTurningFundamentalDriftC2
...
numbers discharged and the perimeter bounds supplied.**  The hypotheses are
those of
`SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2` with the
...
threshold `κ̂·L < 4π`. -/
theorem dist_selInv_le_modulus_of_path_turning_base_uniform_C2 {p q : Data} (Γ : NormalPath p q)
    {c kmin dlt cq kminq dltq kminP P0 P1 : ℝ}
    (hc : 0 < c) (hkmin : 0 < kmin) (hp : IsTubeMember c kmin dlt p)
    (hcq : 0 < cq) (hkminq : 0 < kminq) (hq : IsTubeMember cq kminq dltq q)
    (hkh1 : kh < 1)
    (hP0 : 0 < P0) (hPl : ∀ t, P0 ≤ pathPerim Γ.X t)
    (hPu : ∀ t, pathPerim Γ.X t ≤ P1)
  ...
  exact SelInvPathGaugeBaseC2.dist_selInv_le_modulus_of_path_gauge_base_C2 Γ hc hkmin hp hturnp
    hcq hkminq hq hturnq hP0 hkh1 hXC6 hPl hPu hconst hXper hturn hnu hKn0 hKnk hslit hmark

end SelInvPathTurningBaseUniformC2
