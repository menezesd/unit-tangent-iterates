...
`SelInvFrontVelocityC2.dist_selInv_le_of_front_velocity_C2` still asks
...
`dist_selInv_le_of_front_regularity_C2`.
-/
...
namespace SelInvFrontRegularityC2
...
/-- **The velocity of the family of selected rears splits into the velocity of
the rear tracks and the tangential sliding of the change of variable.** -/
theorem rearOwnVelocity_eq_trackVelocity {X : ℝ → ℝ → ℂ}
    ...
    (hFtangent : ∀ t s, HasDerivAt (frontOfPath X P t)
      (Complex.exp (Complex.I * (angleOfPath V A P t s : ℂ))) s)
    (hΘd : ∀ t s, HasDerivAt (angleOfPath V A P t) (curvOfPath V A P t s) s)
    (hsteer : ∀ t s, HasDerivAt (δ t) (curvOfPath V A P t s - Real.sin (δ t s)) s)
    (t x : ℝ) :
    rearOwnVelocity X V A P δ sf t x
      = trackVelocity (partialTime (frontOfPath X P)) (partialTime (angleOfPath V A P))
          (partialTime δ) (angleOfPath V A P) δ t (sf t x)
        + (partialTime sf t x) • ((Real.cos (δ t (sf t x)) : ℂ)
          * Complex.exp (Complex.I *
            (rearAngle (angleOfPath V A P t) (δ t) (sf t x) : ℂ))) := by
  have hFdiff : Differentiable ℝ (uncurry (frontOfPath X P)) :=
    hFc1.differentiable (by norm_num)
  have hΘdiff : Differentiable ℝ (uncurry (angleOfPath V A P)) :=
    hΘc1.differentiable (by norm_num)
  ...
  have hFa : ∀ t s, HasDerivAt (fun r => frontOfPath X P r s)
      (partialTime (frontOfPath X P) t s) t := hasDerivAt_partialTime hFdiff
  ...
  have hGtime : ∀ t s,
      partialTime (frontParamTrack (frontOfPath X P) (angleOfPath V A P) δ) t s
        = trackVelocity (partialTime (frontOfPath X P))
  ...
  have hGarc : ∀ t s,
      partialArc (frontParamTrack (frontOfPath X P) (angleOfPath V A P) δ) t s
        = (Real.cos (δ t s) : ℂ)
  ...
  exact (hasDerivAt_rearOwnVelocity hFc1 hΘc1 hδC hsfC t x).unique h
...
`SelInvFrontVelocityC2.dist_selInv_le_of_front_velocity_C2` with the four
...
theorem dist_selInv_le_of_front_regularity_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_front_velocity_C2
