...
/-!
# The `C²` estimate with the parameter derivative of the normal speed produced

`SelInvFrontClosingC2.dist_selInv_le_of_front_closing_C2` still carries the
parameter derivative of the normal speed of the path as a datum, assumed to
exist and to be dominated by the cost density.  Neither need be assumed of a
path whose normal speed is `C¹` in the parameter: the derivative is then the
usual one, and the cost density of a normal path already dominates the sup
norms of the first two derivatives of the normal speed
(`NormalPath.le_m_sup`) — the sup norm being reached, since the normal speed of
a closed front is periodic in the parameter (`periodic_eta_of_periodic_front`)
and so is its derivative.

Main results: `periodic_eta_of_periodic_front`, `abs_deriv_eta_le_m`,
`dist_selInv_le_of_front_normalSpeed_C2`.
-/
...
namespace SelInvFrontNormalSpeedC2
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDistSmooth
  ...
  SelInvFrontChangeVarC2 SelInvFrontClosingC2 RearOwnTangential

variable {V A : ℝ → ℝ → ℂ} {δ dn Kn Kdn : ℝ → ℝ → ℝ} {P Pd : ℝ → ℝ}
  {P0 P1 kh Klip Plip : ℝ}

/-- **The normal speed of a path of closed curves is periodic in the
parameter.**  The two slices `r ↦ X r (u+1)` and `r ↦ X r u` are the same
function, so they have the same velocity, and the unit normal is periodic. -/
theorem periodic_eta_of_periodic_front {p q : Data} (Γ : NormalPath p q)
    (hXper : ∀ t, Function.Periodic (Γ.X t) 1)
    (hnuper : ∀ t, Function.Periodic (Γ.nu t) 1) (t : ℝ) :
    Function.Periodic (Γ.eta t) 1 := by
  intro u
  have hfun : (fun r => Γ.X r (u + 1)) = fun r => Γ.X r u := funext fun r => hXper r u
  have h1 := Γ.hasDerivAt_time t (u + 1)
  rw [hfun] at h1
  have hmul : ((Γ.eta t (u + 1) : ℝ) : ℂ) * Γ.nu t (u + 1)
      = ((Γ.eta t u : ℝ) : ℂ) * Γ.nu t u := h1.unique (Γ.hasDerivAt_time t u)
  rw [hnuper t u] at hmul
  have hne : Γ.nu t u ≠ 0 := by
    intro h
    have := Γ.norm_nu t u
    rw [h] at this
    simp at this
  exact Complex.ofReal_inj.mp (mul_right_cancel₀ hne hmul)

/-- **The cost density of a normal path dominates the parameter derivative of
its normal speed**, as soon as that normal speed is `C¹` and periodic: the sup
norm of the derivative is then attained, and the path already dominates it. -/
theorem abs_deriv_eta_le_m {p q : Data} (Γ : NormalPath p q)
    (hetaC1 : ∀ t, ContDiff ℝ (1 : ℕ) (Γ.eta t))
    (hper : ∀ t, Function.Periodic (Γ.eta t) 1) (t u : ℝ) :
    |deriv (Γ.eta t) u| ≤ Γ.m t := by
  have hd : Differentiable ℝ (Γ.eta t) := (hetaC1 t).differentiable (by norm_num)
  have hdper : Function.Periodic (deriv (Γ.eta t)) 1 :=
    ArclengthInverse.periodic_of_hasDerivAt (fun s => (hd s).hasDerivAt) (hper t)
  have hcont : Continuous (deriv (Γ.eta t)) := (hetaC1 t).continuous_deriv (by norm_num)
  have hbdd := ArclengthInverse.bddAbove_abs_of_periodic one_pos hcont hdper
  have hle := MarkedTopology.le_supNorm (f := deriv (Γ.eta t)) hbdd u
  have hs := Γ.le_m_sup t 1 (by norm_num)
  rw [iteratedDeriv_one] at hs
  exact le_trans hle hs

/-- **The `C²` comparison of the two marked selected inverses, with the
parameter derivative of the normal speed produced rather than assumed.**  The
hypotheses are those of
`SelInvFrontClosingC2.dist_selInv_le_of_front_closing_C2` with that derivative
and its bound replaced by the joint requirement that the normal speed be `C¹`
in the parameter. -/
theorem dist_selInv_le_of_front_normalSpeed_C2 {p q : Data} (Γ : NormalPath p q)
    ...
    (hRbd : ∀ t x,
      |frameTangential (rearOwnVelocity Γ.X V A P δ (SelInvFrontChangeVarC2.rearArclengthInv δ)) (rearOwnAngle (angleOfPath V A P) δ (SelInvFrontChangeVarC2.rearArclengthInv δ)) t x| ≤ Rb t)
  ...
  have hnuper : ∀ t, Function.Periodic (Γ.nu t) 1 := by
    intro t u
    rw [hnu t (u + 1), hnu t u, hVper t u]
  have hetaper : ∀ t, Function.Periodic (Γ.eta t) 1 :=
    periodic_eta_of_periodic_front Γ hXper hnuper
  have hetaU : ∀ t u, HasDerivAt (Γ.eta t) (deriv (Γ.eta t) u) u := fun t u =>
    (((hetaC1 t).differentiable (by norm_num)) u).hasDerivAt
  have hetaUbd : ∀ t u, |deriv (Γ.eta t) u| ≤ Γ.m t :=
    abs_deriv_eta_le_m Γ hetaC1 hetaper
  exact dist_selInv_le_of_front_closing_C2
    ...
    hkappa1 hRbd hRbm hr hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontNormalSpeedC2
