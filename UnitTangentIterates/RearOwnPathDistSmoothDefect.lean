...
the defect of its gauge marking
...
conclusions of `RearOwnPathDistGeometricDefect.lean` about the gauge marking `Φ`
in which the pseudodistance is read: it fixes the base point, reads exactly one
...
Main results: `pathDist_and_defect_le_of_front_regularity`,
`pathDist_and_defect_le_of_front_data`.
-/
...
namespace RearOwnPathDistSmoothDefect
...
open UniformFrameBounds GaugePathDistVariable RearOwnPathDist RearOwnFrameDrift
  ...
  RearOwnHigherRegularity RearOwnPathDistSmooth
...
theorem pathDist_and_defect_le_of_front_regularity {p q : Data} (Γ : NormalPath p q) (p' : Data)
  ...
  exact RearOwnPathDistGeometricDefect.pathDist_and_defect_le_of_front_normalVelocity
    Γ p' (Qf' := Qf') (Fdots := Fdots) (dt := dt)
...
front, together with the defect of its gauge marking.**
`RearOwnPathDistSmooth.pathDist_le_of_front_data` with the three extra
conclusions about the marking. -/
theorem pathDist_and_defect_le_of_front_data {p q : Data} (Γ : NormalPath p q) (p' : Data)
    {P0 P1 kh EF : ℝ} {P Qf' : ℝ → ℝ}
    {F Fdot Fdots Ydot : ℝ → ℝ → ℂ}
    {Θ δ K etaF etaFs sf sft dt Θdot w Θdots ws : ℝ → ℝ → ℝ}
    (hP0 : 0 < P0) (hkh0 : 0 ≤ kh) (hkh1 : kh < 1)
  ...
  exact pathDist_and_defect_le_of_front_regularity Γ p' (Qf' := Qf') (Fdots := Fdots) (dt := dt)
    (etaFs := etaFs) (Θdots := Θdots) (ws := ws) (sft := sft) (K := K)
    hP0 hkh0 hkh1 hPl hPu hF hΘ hsteer hstrip0 hstrip1 hdper hK hKc hFper hΘper
...
end RearOwnPathDistSmoothDefect
