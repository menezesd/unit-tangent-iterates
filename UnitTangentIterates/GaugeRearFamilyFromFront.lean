...
theorem exists_variableSpeed_normalPath_of_rearFamily_from_front_with_eta_c2flow_and_c2
  ...
  obtain ⟨Phi, hPhi0, hbase, hflow, hpair⟩ :=
    exists_variableSpeed_normalPath_of_rearFamily_fundamental_with_eta_c2flow_of_intrinsic_bound
    ...
    hangC hkC1 hQpos hQd hclose hangper hxi0 hjac
    (fun a y => abs_frontNormalVelocity_le_cost_density Γ hPpos hlink a y)
    hkappa1 halphaT hkT
  ...
  have hnormal : ContDiff ℝ (2 : ℕ)
      (uncurry (frameNormal Ydot (rearOwnAngle Θ δ sf))) :=
    contDiff_frameNormal (hYdotC.of_le (by norm_num)) (hangC.of_le (by norm_num))
  ...
  have hGammaC2 :=
    FrameNormalSpatialC2Certificate.c2NormalPathData_of_contDiff_frameNormal GammaR
      hnormal hnormalPer hGammaEta hphi1 hphi2 hphi1c hphi2c htrans
  exact ⟨GammaR, hGammaT, hGammaX, hGammaEta, htrans,
    ⟨phi1, phi2, hphi1, hphi2, hphi1c, hphi2c, hphi1bd, hphi2bd⟩,
    hGammam, hGammacost, hGammavar, ⟨hGammaC2⟩⟩
theorem exists_variableSpeed_normalPath_of_rearFamily_from_front_with_eta_c2flow
    {p q : Data} (Γ : NormalPath p q)
    (hkh0 : 0 ≤ kh) (hkh1 : kh < 1)
    (hstrip0 : ∀ t s, 0 ≤ δ t s) (hstrip1 : ∀ t s, δ t s ≤ Real.arcsin kh)
    (hK : ∀ t s, |K t s| ≤ kh)
  ...
  obtain ⟨Phi, hPhi0, hbase, hflow, hpair⟩ :=
    ...
    (hYdotC.of_le (by norm_num)) (hangC.of_le (by norm_num))
    hkC1 hQpos hQd hclose hangper hxi0 hjac
    (fun a y => abs_frontNormalVelocity_le_cost_density Γ hPpos hlink a y)
    hkappa1 halphaT hkT
    halphaTc hkTc halphaTS hmixed hKxbd hKxnn hKxm hkXc hRbd
    (fun t => mul_le_mul_of_nonneg_left (hcostle t) hrnn) hrnn
    hgSd hgSbd hDm hmc hm0 hmstop hmge hnumA hnumK
  exact ⟨Phi, hPhi0, hbase, hflow, hpair.1⟩



theorem exists_variableSpeed_normalPath_of_rearFamily_from_front_with_eta
    {p q : Data} (Γ : NormalPath p q)
    (hkh0 : 0 ≤ kh) (hkh1 : kh < 1)
    (hstrip0 : ∀ t s, 0 ≤ δ t s) (hstrip1 : ∀ t s, δ t s ≤ Real.arcsin kh)
    (hK : ∀ t s, |K t s| ≤ kh)
  ...
  have hcostle : ∀ t, Γ.m t ≤ m t := by
    ...
    rw [le_div_iff₀ hroot]
    nlinarith [Γ.m_nonneg t]
  exact exists_variableSpeed_normalPath_of_rearFamily_fundamental_with_eta
    (Q := fun t => rearArclength (δ t) (P t))
    (Q' := fun t => (∫ u in (0 : ℝ)..P t,
        SelectedChangeOfVariable.cosTimeDeriv δ (RearOwnHigherRegularity.partialTime δ) t u)
      + P' t * Real.cos (δ t (P t)))
    ...
    Γ hkh0 hkh1 hstrip0 hstrip1 hK hF hΘ hsteer hsf hcos hYt hFC hΘC hδC hsfC
    (hYdotC.of_le (by norm_num)) (hangC.of_le (by norm_num))
    hkC1 hQpos hQd hclose hangper hxi0 hjac hPpos hlink hkappa1 halphaT hkT
    halphaTc hkTc halphaTS hmixed hKxbd hKxnn hKxm hkXc hRbd
    (fun t => mul_le_mul_of_nonneg_left (hcostle t) hrnn) hrnn
    hgSd hgSbd hDm hmc hm0 hmstop hmge hnumA hnumK
