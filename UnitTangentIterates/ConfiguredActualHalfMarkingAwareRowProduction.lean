import UnitTangentIterates.ConfiguredMarkingAwareAnalyticKhatCombinedCorrelatedAssembly
...
namespace ConfiguredActualHalfMarkingAwareRowProduction
...
def core (P : RowPackage O) :
  ...
  ConfiguredMarkingAwareAnalyticKhatConstructionCoreProvider.constructionCore
    ...
    P.a P.MA P.NA P.K0 P.K1 P.K2 P.source P.provider
...
structure CoherentPackage (MA0 NA0 : ℝ) where
  ...
  caps : CapFamily (RowPackage.core scalar row)
    (shiftSequence
    ...
    Nonempty (
      Σ X : TriangularMarkedPathSchemeVariableTerminal.LimitOutput
        ...
        PaperFacingVariableTerminalOutput.Output X
          ...
          ((D P.scalar).Hs 0)) := by
  apply ConfiguredMarkingAwareAnalyticKhatCombinedCorrelatedAssembly.conclude
    P.scalar.large P.scalar.pair P.scalar.model_data
    (RowPackage.core P.scalar P.row) P.caps
...
end ConfiguredActualHalfMarkingAwareRowProduction
