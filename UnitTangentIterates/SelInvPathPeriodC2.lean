...
`SelInvPathCurvatureC2.exists_steering_dist_selInv_le_of_path_curvature_C2`
...
Main results: `contDiff_period_of_path`,
`exists_steering_dist_selInv_le_of_path_period_C2`.
-/
...
/-- **The arclength period of the slices of a path is as regular as the path.**
It is the norm of the velocity at the marked point, which does not vanish. -/
theorem contDiff_period_of_path {X : ℝ → ℝ → ℂ} {n : ℕ}
    (hV : ContDiff ℝ (n : ℕ) (uncurry (pathVel X)))
    (hne : ∀ t, pathVel X t 0 ≠ 0) :
    ContDiff ℝ (n : ℕ) fun t => ‖pathVel X t 0‖ := by
  have hcomp : ContDiff ℝ (n : ℕ) fun t => pathVel X t 0 :=
    hV.comp (contDiff_id.prodMk contDiff_const)
  exact hcomp.norm ℝ hne
...
those of
`SelInvPathCurvatureC2.exists_steering_dist_selInv_le_of_path_curvature_C2` with
...
theorem exists_steering_dist_selInv_le_of_path_period_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact exists_steering_dist_selInv_le_of_path_curvature_C2
