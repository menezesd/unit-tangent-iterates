...
theorem exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta_c2flow
    ...
    ∃ Γ : NormalPath a b, Γ.T = T ∧ (∀ t u, Γ.X t u = Y t (Phi t u)) ∧
      ...
      (∀ t u, HasDerivAt (Phi t) (flowDeriv hx Phi (Q 0) t u) u) ∧
      (∀ t u, HasDerivAt (fun v => flowDeriv hx Phi (Q 0) t v)
        (flowDeriv2 hx hxx Phi (Q 0) t u) u) ∧
      (∀ t, Continuous (fun u => flowDeriv hx Phi (Q 0) t u)) ∧
      (∀ t, Continuous (fun u => flowDeriv2 hx hxx Phi (Q 0) t u)) ∧
      (∀ t u, flowDeriv hx Phi (Q 0) t u ≤ P1) ∧
      (∀ t u, |flowDeriv2 hx hxx Phi (Q 0) t u| ≤ G1) ∧
  ...
  have htrans : ∀ t u, Phi t (u + 1) = Phi t u + Q t := fun t u =>
    GaugeFlowVariablePeriod.flow_translation_var hlip hQd hqp hPhid hPhi0 u t
  have hphi1 : ∀ t u, HasDerivAt (Phi t) (flowDeriv hx Phi (Q 0) t u) u :=
    fun t u => hasDerivAt_flow_initial hlip hcont hPhid (hQpos 0) hPhi0 hxd u t
  have hphi2 : ∀ t u, HasDerivAt (fun v => flowDeriv hx Phi (Q 0) t v)
      (flowDeriv2 hx hxx Phi (Q 0) t u) u :=
    fun t u => hasDerivAt_flowDeriv hlip hcont hPhid (hQpos 0) hPhi0 hxd hxcont hxxd
      hxxcont hxxK u t
  have hphi1c : ∀ t, Continuous (fun u => flowDeriv hx Phi (Q 0) t u) :=
    fun t => FlowJointContinuity.continuous_flowDeriv_initial hlip hPhid hPhi0 hxcont t
  have hphi2c : ∀ t, Continuous (fun u => flowDeriv2 hx hxx Phi (Q 0) t u) :=
    fun t => FlowJointContinuity.continuous_flowDeriv2_initial
      hlip hPhid hPhi0 hxcont hxxcont t
  refine ⟨Γ, hΓT, hΓX, hΓeta, htrans, hphi1, hphi2, hphi1c, hphi2c, hP1, hG1, hΓm,
    by rw [hΓcost], ?_⟩
...
theorem exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta
    {a b : Data} {Y : ℝ → ℝ → ℂ} {alpha k en h hx hxx Phi : ℝ → ℝ → ℝ}
    {alphaT kT kX : ℝ → ℝ → ℝ} {C C2 A Kt Kx Rb m Q Q' : ℝ → ℝ} {K K2 : NNReal}
    {turn T P0 P1 khat G1 Cg : ℝ}
    -- the slices, parametrized by their own arclength, and their motion
...
`exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta`. -/
theorem exists_variableSpeed_normalPath_of_gauge_marking_fundamental
  ...
  obtain ⟨Γ, hΓT, hΓX, -, hΓm, hΓcost, hΓvar⟩ :=
    exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta
      ...
      hKxnn hcostA hcostK hQpos hQd hqp hkper halphaper hbase hk hC hC2 hAbd hKtbd
      hKxbd hRbd hT hencont hstart hfinish hmc hm0 hmstop hmbd hmsup
