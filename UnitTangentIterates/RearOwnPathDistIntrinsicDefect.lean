...
`Φ` fixes the base point, reads exactly one rear period, and deviates from the
affine marking of the terminal period by at most `2 P₁ κ̂/(1 − κ̂²) · cost Γ` —
under the single extra hypothesis that the base drift of the front vanishes.

Main result: `pathDist_and_defect_le_of_front_intrinsic`.
-/
...
namespace RearOwnPathDistIntrinsicDefect
...
removed, together with the defect of its gauge marking.**
`RearOwnPathDistIntrinsic.pathDist_le_of_front_intrinsic` with the three extra
conclusions about the marking. -/
theorem pathDist_and_defect_le_of_front_intrinsic {p q : Data} (Γ : NormalPath p q) (p' : Data)
  ...
  obtain ⟨Phi, hPhi0, hbase, hone, hdefect, hPhi⟩ :=
    RearOwnPathDistCurvatureDefect.pathDist_and_defect_le_of_front_curvature Γ p'
      (Qf' := Qf') (EF := EF)
...
end RearOwnPathDistIntrinsicDefect
