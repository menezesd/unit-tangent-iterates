...
`SelInvFrontChangeVarFundamentalC2.dist_selInv_le_of_front_changevar_fundamental_C2` still asks that
...
This is the fundamental-domain form: the bound on the tangential drift is
asked on one rear period only.

Main result: `dist_selInv_le_of_front_closing_fundamental_C2`.
-/
...
namespace SelInvFrontClosingFundamentalC2
...
`SelInvFrontChangeVarFundamentalC2.dist_selInv_le_of_front_changevar_fundamental_C2` with the
periodicity of the normal velocity of the rears removed. -/
theorem dist_selInv_le_of_front_closing_fundamental_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  have hper : ∀ t, Function.Periodic
    ...
    exact (GaugeClosingRelations.closing_relations hYC hYx hYtframe htau0 htauper hclose
      hQd t x).2
  exact SelInvFrontChangeVarFundamentalC2.dist_selInv_le_of_front_changevar_fundamental_C2
...
end SelInvFrontClosingFundamentalC2
