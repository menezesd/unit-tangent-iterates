...
/-!
# The base-point form of the `C²` estimate is not vacuous

`SelInvPathTubeBaseC2.dist_selInv_le_modulus_of_path_tube_base_C2` states the
`C²` comparison of the two marked selected inverses under hypotheses that are
purely geometric on the path: the tube membership of the two ends is produced
from the pinching of the slices, and nothing is assumed about the tangential
drift of the family of selected rears.  This file checks that hypothesis block
on the resting circle of radius `r > 1` of `SelInvPathTurningCircle.lean`.

Main result: `restCircle_tube_base_instance`.
-/
...
namespace SelInvPathTubeBaseCircle
...
/-- **The hypothesis block of the base-point form of the `C²` estimate is
consistent.**  Every hypothesis of
`SelInvPathTubeBaseC2.dist_selInv_le_modulus_of_path_tube_base_C2` holds for
the resting circle of radius `r > 1`; in particular the marked circle carries
its velocity and its acceleration as the derivatives of its curve, and its
marked point is at rest. -/
theorem restCircle_tube_base_instance (hr : 1 < r) :
    ...
    ∃ dn δ : ℝ → ℝ → ℝ,
      ...
      dist (SelectedInverseMap.selInv (1 / r) (circleData r)) (SelectedInverseMap.selInv (1 / r) (circleData r))
        ≤ selInvFrontModulus P1 (1 / r) (perim (SelectedInverseMap.selInv (1 / r) (circleData r)))
            ...
            (2 * (1 / r) / Real.sqrt (1 - (1 / r) ^ 2) ^ 3) (pathPv0 (1 / r) P0 khat (P1 * ((1 / r) / (1 - (1 / r) ^ 2)))) khat (jacobiSourceConst (1 / r) P0)
  ...
  exact dist_selInv_le_modulus_of_path_tube_base_C2 (restCirclePath r)
