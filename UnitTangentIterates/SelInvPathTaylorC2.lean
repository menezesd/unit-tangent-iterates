...
`SelInvPathSteeringC2.dist_selInv_le_of_path_steering_C2` still carries, besides
...
Main result: `dist_selInv_le_of_path_taylor_C2`, the same `C²` comparison with
`Kdn`, `Pd` and the six constants all removed, at the cost of one extra
derivative of the normalized curvature (`Kn` jointly `C⁴` instead of `C³`,
which is what the statement already asks of the period).
-/
...
`SelInvPathSteeringC2.dist_selInv_le_of_path_steering_C2` with the time
...
theorem dist_selInv_le_of_path_taylor_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact dist_selInv_le_of_path_steering_C2 (Kdn := partialT Kn) (Pd := deriv P)
