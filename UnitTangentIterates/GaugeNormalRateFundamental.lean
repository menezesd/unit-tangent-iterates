...
theorem exists_variableSpeed_normalPath_of_normal_rate_fundamental_with_eta_c2flow
    ...
    ∃ Γ : NormalPath a b, Γ.T = T ∧ (∀ t u, Γ.X t u = Y t (Phi t u)) ∧
      ...
      (∀ t u, HasDerivAt (Phi t) (FlowDerivative.flowDeriv hx Phi (Q 0) t u) u) ∧
      (∀ t u, HasDerivAt (fun v => FlowDerivative.flowDeriv hx Phi (Q 0) t v)
        (GaugeFlowTimeDerivative.flowDeriv2 hx hxx Phi (Q 0) t u) u) ∧
      (∀ t, Continuous (fun u => FlowDerivative.flowDeriv hx Phi (Q 0) t u)) ∧
      (∀ t, Continuous (fun u => GaugeFlowTimeDerivative.flowDeriv2 hx hxx Phi (Q 0) t u)) ∧
      (∀ t u, FlowDerivative.flowDeriv hx Phi (Q 0) t u ≤ P1) ∧
      (∀ t u, |GaugeFlowTimeDerivative.flowDeriv2 hx hxx Phi (Q 0) t u| ≤ G1) ∧
  ...
  exact exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta_c2flow
...
theorem exists_variableSpeed_normalPath_of_normal_rate_fundamental_with_eta
    (hYC1 : ContDiff ℝ 1 (uncurry Y))
    (hY : ∀ t s, HasDerivAt (Y t) (Complex.exp (Complex.I * (alpha t s : ℂ))) s)
    (hYt : ∀ t s, HasDerivAt (fun r => Y r s)
      (((-h t s : ℝ) : ℂ) * Complex.exp (Complex.I * (alpha t s : ℂ))
  ...
  have hKtbd : ∀ t, ∀ x ∈ Icc (0 : ℝ) (Q t),
    ...
    simpa using hRbd t x hx
  have hkhat : 0 ≤ khat := le_trans (abs_nonneg _) (hk 0 0 ⟨le_rfl, (hQpos 0).le⟩)
  exact exists_variableSpeed_normalPath_of_gauge_marking_fundamental_with_eta
    (A := angleRateBound S1 Rb khat) (Kt := curvRateBound S0 S2 Rb Kx khat)
    hYC1 hY hYt halpha hlip hcont hPhid hPhi0 hxd hxcont hxxd hxxcont hxxK hP1 hG1
    hCnn hC2nn hcost hcost2 halphaC1 hkC1 halphaT hkT hkX halphaTc hkTc hkXc hkc hKxnn
    (fun t => angleRate_le_cost hS1m hRbm hm0 hkhat hnumA t)
...
/-- Strengthened normal-rate path projected to the historical result shape. -/
