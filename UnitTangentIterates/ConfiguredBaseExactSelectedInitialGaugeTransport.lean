...
namespace ConfiguredBaseExactSelectedInitialGaugeTransport
...
def qRate (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) (t : ℝ) : ℝ := -xi P t (G.q t)

abbrev shiftedRear (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℂ :=
  TimeDependentSpatialReanchoring.shift (rearR W S) G.q

abbrev shiftedPsi (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ :=
  RearOwnFrameGaugeFlowReanchoring.shiftedPsi (psiR W S) G.q

abbrev shiftedKappa (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ :=
  TimeDependentSpatialReanchoring.shift (kappaR W S) G.q

abbrev shiftedJacobiSource (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ :=
  TimeDependentSpatialReanchoring.shift (jacobiSource W S) G.q

abbrev shiftedCurvatureSpatial (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ :=
  TimeDependentSpatialReanchoring.shift (curvatureSpatial W S) G.q

def Ydot (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℂ :=
  RearOwnFrameGaugeFlowReanchoring.shiftedYdot P.Ydot (psiR W S) (xi P) G.q

def alphaT (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ := fun t x ↦
  TimeDependentSpatialReanchoring.shift P.alphaT G.q t x + qRate P G t * shiftedKappa P G t x

def kT (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ := fun t x ↦
  TimeDependentSpatialReanchoring.shift P.kT G.q t x + qRate P G t * shiftedCurvatureSpatial P G t x

abbrev gS (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) : ℝ → ℝ → ℝ := TimeDependentSpatialReanchoring.shift P.gS G.q
...
structure ShiftedTransport (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P)) where
...
variable {W : Output D Q n A} {S : ExactSelected (n := n) H}
  (P : PreTransport W H S) (G : RearOwnFrameGaugeFlowInitialValue.Gauge (xi P))
...
end ConfiguredBaseExactSelectedInitialGaugeTransport
