...
the defect of the gauge marking of the terminal end
...
Main result: `exists_marked_rearOwn_pathDist_and_defect_terminal`.
-/
...
namespace SelectedInverseRearOwnTerminalDefect
...
defect of the gauge marking of the terminal end.**
...
`2 P₁ κ̂/(1 − κ̂²) · cost Γ`. -/
theorem exists_marked_rearOwn_pathDist_and_defect_terminal {p q : Data} (Γ : NormalPath p q)
  ...
  obtain ⟨-, -, -, hperimp, -, -, EF, hEF0, hEFbd, Phi, hPhi0, hbase, -, hdefect, hPhi⟩ :=
    SelectedInverseRearOwnPathDefect.exists_marked_rearOwn_pathDist_and_defect Γ
  ...
  refine ⟨hperimp, hperimq, EF, hEF0, hEFbd, Phi, fun u => by rw [hPhi0 u, hperimp],
    hbase, fun u => by rw [hperimq]; exact hdefect u, ?_⟩
  intro q' hq'
  refine hPhi q' fun u => ?_
  rw [hq' u, ← hevq (Phi Γ.T u), ev]

end SelectedInverseRearOwnTerminalDefect
