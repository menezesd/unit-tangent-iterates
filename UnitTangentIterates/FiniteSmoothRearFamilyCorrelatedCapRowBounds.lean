...
/-!
# Physical row bounds from correlated terminal caps

This module supplies the geometric component intentionally left open in
`CapFamily.toPhysicalProducer`.  A row terminal is controlled by the sum of
the marked-column radius and its actual terminal marking defect.  The
configured row budget then gives one fixed tube and uniform perimeter and
acceleration bounds at every depth of a fixed row.
-/
...
/-- Distance from the configured model to every retained physical row. -/
theorem CapFamily.retained_dist_le
  ...
  | zero =>
      ...
      have htail := ShadowingTails.tail_nonneg
        (fun j => hdiag (n + j)) 0
      rw [dist_self]
      exact mul_nonneg
        (add_nonneg (hbase n) (hendpoint n)) htail
  | succ k =>
...
/-- The marking defect from a retained physical row to its selected column
fits in the same combined radius. -/
theorem CapFamily.endpoint_dist_le
  ...
  | zero =>
      ...
      rw [dist_self]
      exact mul_nonneg
        (add_nonneg (hbase n) (hendpoint n))
...
/-- A correlated terminal-cap family plus the scalar row budget constructs
all rowwise physical geometry; no separate physical-bounds callback remains. -/
