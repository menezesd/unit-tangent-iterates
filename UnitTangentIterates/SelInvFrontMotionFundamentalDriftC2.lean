...
`SelInvFrontStripFundamentalDriftC2.dist_selInv_le_of_front_strip_fundamental_drift_C2` still asks the caller for
...
* `dist_selInv_le_of_front_motion_fundamental_drift_C2` — the estimate with `alphaT`, `kT` and
...
namespace SelInvFrontMotionFundamentalDriftC2
...
theorem dist_selInv_le_of_front_motion_fundamental_drift_C2 {p q : Data} (Γ : NormalPath p q)
  ...
  exact SelInvFrontStripFundamentalDriftC2.dist_selInv_le_of_front_strip_fundamental_drift_C2
    ...
    hxi0 hetaU hetaUbd hnumA hnumK hsmall

end SelInvFrontMotionFundamentalDriftC2
